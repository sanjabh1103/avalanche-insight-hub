...
class ChangeDetectionSwinUNet(nn.Module):
    def __init__(
        ...
        six_channel: bool = False,       # F3: mSAFE 6-channel single-encoder mode
    ) -> None:
        """
        ...
        F3: When six_channel=True, uses a single encoder with 6 input channels
        (VV_diff, VH_diff, VV_VH_prod, coherence, slope, PAR) instead of the
        bi-temporal dual-encoder path.
        """
        super().__init__()
        ...
        # F3: In six_channel mode, use a single encoder with 6 input channels
        ...
        if use_aux and not six_channel:
    ...
    def forward(
        ...
        if self.six_channel:
            # F3: Single-encoder 6-channel path. x1 is the 6-channel tensor.
            f1_list = self.sar_encoder(x1)
            fused_features = list(f1_list)
        else:
            f1_list = self.sar_encoder(x1)
    ...
    @torch.no_grad()
    def predict(self, x1: torch.Tensor, x2: torch.Tensor | None = None, aux: torch.Tensor | None = None) -> torch.Tensor:
        self.eval()
