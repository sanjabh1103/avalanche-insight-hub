...
from backend.common.cold_start_config import ColdStartConfig, validate_cold_start_eligible
...
SURROGATE_TEMP_GRADIENT_MAX = 0.8
SURROGATE_TEMP_GRADIENT_MIN = -0.2
SURROGATE_HEAVY_SNOWFALL_CM = 10.0
SURROGATE_WARM_TEMP_SNOWFALL_C = 10.0
SURROGATE_NORTHNESS_WET_THRESHOLD = 0.7
SURROGATE_WET_ACTIVATION_HIGH = 0.7
SURROGATE_WET_ACTIVATION_COLD_TEMP_C = -10.0
SURROGATE_WET_ACTIVATION_COLD_THRESHOLD = 0.6
...
def physically_valid_surrogate_rows(frame: pd.DataFrame) -> pd.Series:
    ...
    # Rule 6: Snowpack temperature gradient bounds — gradients outside
    # [-0.2, 0.8] °C/m are physically implausible for snowpack.
    if 'temp_gradient' in frame.columns:
        temp_gradient = _finite_column(frame, 'temp_gradient')
        valid &= temp_gradient.between(SURROGATE_TEMP_GRADIENT_MIN, SURROGATE_TEMP_GRADIENT_MAX) | temp_gradient.isna()
    # Rule 7: Heavy snowfall at warm temperatures — snowfall > 10 cm at
    # temperatures > 10°C is physically implausible (snow would melt).
    if {'snowfall_24h', 'temperature_2m'}.issubset(frame.columns):
        valid &= ~(
            (snowfall > SURROGATE_HEAVY_SNOWFALL_CM)
            & (temperature > SURROGATE_WARM_TEMP_SNOWFALL_C)
        )
    # Rule 8: North-facing aspect with wet activation — strongly north-facing
    # slopes (northness > 0.7) with high wet activation signal (> 0.7) are
    # physically inconsistent; north aspects receive minimal solar radiation
    # and rarely experience wet-snow activation in winter.
    if {'northness', 'wet_activation_signal'}.issubset(frame.columns):
        northness = _finite_column(frame, 'northness')
        wet_activation = _finite_column(frame, 'wet_activation_signal')
        valid &= ~(
            (northness > SURROGATE_NORTHNESS_WET_THRESHOLD)
            & (wet_activation > SURROGATE_WET_ACTIVATION_HIGH)
        )
    # Rule 9: Wet activation at very cold temperatures — wet_activation_signal
    # > 0.6 at temperatures < -10°C is physically implausible; liquid water
    # cannot exist in snow at such cold temperatures, so wet-snow activation
    # is impossible. This is a seasonal consistency check that rejects
    # synthetic samples implying wet-snow conditions during deep winter cold.
    if {'wet_activation_signal', 'temperature_2m'}.issubset(frame.columns):
        valid &= ~(
            (wet_activation > SURROGATE_WET_ACTIVATION_COLD_THRESHOLD)
            & (temperature < SURROGATE_WET_ACTIVATION_COLD_TEMP_C)
        )
...
def spatial_group_cv_pss(frame: pd.DataFrame, seed: int, n_splits: int, feature_columns: list[str]) -> dict[str, Any]:
    """Evaluate PSS across spatial folds (GroupKFold by region_key) to test
    whether the model generalizes to regions unseen during training."""
    if 'region_key' not in frame.columns:
        return {'mean_pss': 0.0, 'fold_pss': [], 'note': 'no_region_key_column'}
    groups = frame['region_key'].astype(str).values
    unique_groups = list(set(groups))
    n_groups = len(unique_groups)
    if n_groups < 2:
        return {'mean_pss': 0.0, 'fold_pss': [], 'note': 'insufficient_regions_for_spatial_cv'}
    x = frame[feature_columns].astype(float).values
    y = frame['label'].astype(int).values
    if len(y) < 4 or len(np.unique(y)) < 2:
        return {'mean_pss': 0.0, 'fold_pss': [], 'note': 'insufficient_samples_or_classes'}
    actual_splits = min(n_splits, n_groups)
    gkf = GroupKFold(n_splits=actual_splits)
    fold_scores: list[float] = []
    fold_regions: list[list[str]] = []
    for fold_idx, (train_idx, test_idx) in enumerate(gkf.split(x, y, groups)):
        y_train_fold = y[train_idx]
        y_test_fold = y[test_idx]
        if len(np.unique(y_train_fold)) < 2 or len(np.unique(y_test_fold)) < 2:
            fold_regions.append(sorted(set(groups[test_idx])))
            continue
        rf_fold = RandomForestClassifier(
            n_estimators=SURROGATE_CV_RF_TREES,
            random_state=seed + fold_idx,
        ...
        rf_fold.fit(x[train_idx], y_train_fold)
        y_prob_fold = rf_fold.predict_proba(x[test_idx])[:, 1]
        fold_pss, _ = peirce_skill_score_max(y_test_fold, y_prob_fold)
        fold_scores.append(fold_pss)
        fold_regions.append(sorted(set(groups[test_idx])))
    ...
    return {
        'mean_pss': mean_pss,
        'fold_pss': fold_scores,
        'n_splits': actual_splits,
        'n_regions': n_groups,
        'held_out_regions_per_fold': fold_regions,
        'note': 'spatial_generalization_test' if fold_scores else 'all_folds_skipped',
    }
...
def compute_tree_shap(
    ...
    *,
    ...
    shap_values = (
        explainer.shap_values(selected_frame, approximate=True)
        if approximate
        else explainer.shap_values(selected_frame)
    )
...
def fit_surrogate_bundle(
    *,
    ...
    spatial_cv_metrics = spatial_group_cv_pss(frame, seed=seed, n_splits=time_series_splits, feature_columns=feature_columns)
    ...
    metrics = {
        ...
        'pss_spatial_mean': spatial_cv_metrics.get('mean_pss', 0.0),
        'pss_spatial_folds': spatial_cv_metrics.get('fold_pss', []),
        'spatial_cv_note': spatial_cv_metrics.get('note', ''),
        'spatial_cv_n_regions': spatial_cv_metrics.get('n_regions', 0),
    ...
    return {
        ...
        'spatial_cv_metrics': spatial_cv_metrics,
        'selector': selector,
...
def fit_cold_start_bundle(
    *,
    ...
    """Train a data-efficient RF model for cold-start deployment.

    Key differences from fit_surrogate_bundle:
    - More aggressive SVM-RFE: targets config.target_feature_count (default 10) features
    - Stronger class weight ({0:1, 1:6} vs {0:1, 1:4})
    - Fewer RF trees (200 vs 300) and higher min_samples_leaf (3 vs 2) to prevent overfitting
    - Falls back to class_weight_only resampling more readily (small data)
    - Tags bundle with forecast_mode='cold_start'
    """
    eligible, eligibility_msg = validate_cold_start_eligible(frame)
    if not eligible:
        print(f'[cold_start] WARNING: {eligibility_msg}', flush=True)
    else:
        print(f'[cold_start] {eligibility_msg}', flush=True)

    cv_metrics = timeseries_cv_pss(frame, seed=seed, n_splits=3, feature_columns=feature_columns)
    spatial_cv_metrics = spatial_group_cv_pss(frame, seed=seed, n_splits=3, feature_columns=feature_columns)
    ...
    effective_n_features = min(config.target_feature_count, x_res.shape[1])
    selector = RFE(
        estimator=SVC(kernel='linear', class_weight='balanced', random_state=seed),
        n_features_to_select=effective_n_features,
        step=1,
    ...
    selected_features = [feature for feature, keep in zip(feature_columns, selector.support_) if keep]
    if not selected_features:
        selected_features = feature_columns[:config.target_feature_count]
    ...
    metrics = {
        ...
        'roc_auc': float(roc_auc_score(y_test, y_prob)) if len(np.unique(y_test)) >= 2 else None,
        'selected_feature_count': len(selected_features),
        'target_feature_count': config.target_feature_count,
        'pss_holdout': holdout_pss,
        'pss_optimal_threshold': optimal_threshold,
        'pss_holdout_at_threshold_0p5': peirce_skill_score(y_test, y_pred),
        'f1_at_optimal_threshold': float(f1_score(y_test, y_pred_optimal, zero_division=0)),
        ...
        'spatial_cv_n_regions': spatial_cv_metrics.get('n_regions', 0),
        'raw_mean_prob_p99': float(np.quantile(raw_mean_prob, 0.99)) if raw_mean_prob.size else None,
        'pss_floor_applied': config.pss_floor,
        'brier_ceiling_applied': config.brier_ceiling,
    }
    ...
    return {
        ...
        'cold_start_config': {
            'target_feature_count': config.target_feature_count,
            'pss_floor': config.pss_floor,
            'brier_ceiling': config.brier_ceiling,
            'rf_trees': config.rf_trees,
            'min_samples_leaf': config.min_samples_leaf,
            'class_weight': dict(config.class_weight),
        },
    }
