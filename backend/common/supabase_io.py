...
def _configured_credentials() -> tuple[str | None, str | None]:
    """Resolve REST credentials without importing the heavyweight app config.

    The minimal SNOWPACK Docker runtime only needs this lightweight REST client;
    importing ``backend.common.config`` would also import the ML artifact
    stack.  Keep the normal settings fallback for application/tests that
    patch ``load_settings`` or provide only one environment variable.
    """
...
def _headers() -> dict[str, str]:
    _, service_role_key = _configured_credentials()
