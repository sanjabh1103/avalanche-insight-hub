...
def european_source_registry() -> dict[str, EuropeanSource]:
    sources = [
        ...
        _source(
            ...
            label='HiAVAL Himalayan avalanche event database',
            region_keys=(
                'himalayas_nepal',
            ...
            data_lane=OCCURRENCE_LABELS_LANE,
            record_count=746,
            record_count_kind='open event database rows; exact-date/AOI subset is materialized by the MVP4 adapter',
            curation_level='open literature, report, media, and local-interaction event database',
            license='CC BY 4.0; preserve attribution and source references',
            source_url='https://github.com/fidelsteiner/HiAVAL',
            citation='Steiner et al. (2023). HiAVAL: A database of avalanche events in High Mountain Asia. NHESS 23, 2569–2584. https://doi.org/10.5194/nhess-23-2569-2023',
            recommended_weight=0.7,
            risk_notes=(
                'The database is incomplete and reporting/impact biased; it is not exhaustive occurrence truth.',
                'Only exact-date, coordinate-valid rows enter the MVP4 core snapshot; partial dates remain excluded.',
                'Rows must be overlap-audited against gee_sar before they count as an independent positive source.',
            ),
        ),
        _source(
            source_key='everest_sentinel1',
            label='Everest Sentinel-1 manually updated avalanche-deposit outlines',
            region_keys=('himalayas_nepal',),
            data_lane=SAR_DETECTION_ACTIVITY_LANE,
            record_count=5115,
            record_count_kind='manually updated polygon detections bounded by Sentinel-1 scene-pair dates',
            curation_level='manual satellite-derived polygon review with bounded detection intervals',
            license='CC BY 4.0; preserve attribution and source references',
            source_url='https://zenodo.org/records/10895011',
            citation="Kneib et al. (2024). Data for 'Mapping and characterization of avalanches on mountain glaciers with Sentinel-1 satellite imagery'. Zenodo. https://doi.org/10.5281/zenodo.10895011",
            allowed_roles=(REGISTRY_ONLY_ROLE, STAGING_ROLE, BENCHMARK_ROLE),
            default_training_role=BENCHMARK_ROLE,
            recommended_weight=0.4,
            risk_notes=(
                'Each record is bounded by two Sentinel-1 acquisition dates; the exact avalanche time is unknown.',
                'Do not substitute the interval start for an event timestamp in weather or snowpack joins.',
                'The Everest subset covers a Nepal glacier subregion and is not representative of all Himalayan avalanche activity.',
            ),
        ),
        _source(
            source_key='bipad_nepal_avalanche_candidate',
            label='BIPAD Nepal avalanche incident API candidate',
            region_keys=('himalayas_nepal',),
            data_lane=OCCURRENCE_LABELS_LANE,
            record_count=46,
            record_count_kind='public API incident rows; target AOI subset is materialized by the offline adapter',
            curation_level='government disaster incident API with verified/approved fields',
            license='BIPAD API reuse/model-training terms pending written review',
            source_url='https://bipadportal.gov.np/api/',
            citation='BIPAD API incident endpoint, hazard=3 avalanche candidate records.',
            allowed_roles=(REGISTRY_ONLY_ROLE, STAGING_ROLE, BENCHMARK_ROLE),
            default_training_role=BENCHMARK_ROLE,
            recommended_weight=0.3,
            risk_notes=(
                'The live API is mutable; only a hash-pinned offline response is reproducible evidence.',
                'Current incidentOn values are predominantly day-resolution midnight values and do not prove exact event time.',
                'Written reuse/model-training terms and deduplication against HiAVAL are required before any core-training consideration.',
            ),
        ),
        _source(
...
def normalize_staged_european_record(
    ...
    *,
    ...
    return {
        ...
        'event_time_start': _clean_string(raw.get('event_time_start')) or None,
        'event_time_end': _clean_string(raw.get('event_time_end')) or None,
        'timestamp_precision': _clean_string(raw.get('timestamp_precision')) or None,
        'event_group_id': _clean_string(raw.get('event_group_id')) or None,
        'origin_source_family': _clean_string(raw.get('origin_source_family')) or None,
