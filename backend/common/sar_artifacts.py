...
def persist_sar_artifacts(
    ...
    if not artifacts or not has_supabase_credentials():
        return 0
    # Artifact writes must be idempotent with the event retry contract. The
    # forward migration adds the unique avalanche_event_id index; a plain
    # insert would duplicate artifacts after a crash before chunk completion.
    rest_upsert(
        'sar_detection_artifacts',
        artifacts,
        on_conflict='avalanche_event_id',
    )
