...
# F2: Zone one-hot features for Regional Attention MTS-LSTM
# 4 Himalayan microclimate zones. All zeros when zone_type is None (backward compatible).
ZONE_TYPES = ['pir_panjal', 'shamshabari', 'great_himalaya', 'karakoram_ladakh']
ZONE_ONEHOT_FEATURES = ['zone_pir_panjal', 'zone_shamshabari', 'zone_great_himalaya', 'zone_karakoram_ladakh']


def extract_zone_onehot(zone_type: str | None) -> np.ndarray:
    """Return 4-dim one-hot vector for zone_type. All zeros when None or unknown."""
    vec = np.zeros(len(ZONE_TYPES), dtype=np.float32)
    if zone_type and zone_type in ZONE_TYPES:
        vec[ZONE_TYPES.index(zone_type)] = 1.0
    return vec
...
def build_training_branch_arrays(
    ...
    *,
    ...
    for _, row in frame.iterrows():
        ...
        base_static = extract_static_vector(row, static_features=static_names)
        # F2: Append zone one-hot to static vector
        zone_type = row.get('zone_type') if isinstance(row, pd.Series) else row.get('zone_type')
        zone_onehot = extract_zone_onehot(zone_type if isinstance(zone_type, str) else None)
        static_rows.append(np.concatenate([base_static, zone_onehot]))
...
def build_inference_branches(
    *,
    ...
    base_static = extract_static_vector(static_feature_row, static_features=static_names)
    # F2: Append zone one-hot to static vector
...
def build_edmrg_sequence_branches(
    *,
    edmrg_records: list[Any],
    terrain: dict[str, float],
    static_feature_row: dict[str, float],
    zone_type: str | None = None,
    hourly_steps: int = DEFAULT_HOURLY_STEPS,
    ...
    """F20: Build SequenceBranches from eDMRG telemetry records.

    Accepts EdmrgRecord objects from ``backend.common.edmrg_adapter``.
    Converts to weather sample dicts and delegates to
    ``sequence_matrix_from_samples`` for the same processing as
    the Open-Meteo pipeline.
    """
    from backend.common.edmrg_adapter import edmrg_to_weather_samples

    weather_samples = edmrg_to_weather_samples(edmrg_records)
    feature_names = dynamic_features or DYNAMIC_SEQUENCE_FEATURES
    static_names = static_features or STATIC_SEQUENCE_FEATURES
    hourly_matrix, daily_matrix = sequence_matrix_from_samples(
