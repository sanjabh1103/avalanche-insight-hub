...
class SnowpackProxy:
    ...
    quality_flags: tuple[str, ...] = ('proxy', 'uncalibrated')
    run_id: str | None = None
    execution_status: str = 'fallback_proxy'
    official_warning_eligible: bool = False
    is_official_warning: bool = False


@dataclass(frozen=True)
class SnowpackProxyBatchResult:
    proxy: SnowpackProxy | None
    status: str
    error: str | None = None


def winter_season_start(as_of: datetime, season_start_str: str | None = None) -> date:
    ...
    When season_start_str is provided (MM-DD format, e.g. "10-15" for Oct 15),
    uses that date instead of Nov 1. This supports zone-specific season
    starts such as Karakoram/Ladakh where accumulation begins earlier.
    """
    month = 11
    day = 1
    if season_start_str:
        parts = season_start_str.split('-')
        if len(parts) == 2:
            try:
                month = int(parts[0])
                day = int(parts[1])
                ...
                pass
    if as_of.month > month or (as_of.month == month and as_of.day >= day):
        return date(as_of.year, month, day)
...
def _proxy_to_cache_value(proxy: SnowpackProxy) -> dict[str, Any]:
    return {
        ...
        'method': proxy.method,
        'source_class': proxy.source_class,
        'source': proxy.source,
        'uncertainty': proxy.uncertainty,
        'quality_flags': list(proxy.quality_flags),
        'run_id': proxy.run_id,
        'execution_status': proxy.execution_status,
...
def _proxy_from_cache_value(value: Any) -> SnowpackProxy | None:
    ...
    try:
        numeric_values = (
            value['estimated_shear_strength'],
            value['snow_settlement_index'],
        )
        if any(
            isinstance(raw, bool)
        ...
        if raw_uncertainty is not None and (
            isinstance(raw_uncertainty, bool)
            or not isinstance(raw_uncertainty, (int, float))
            or not math.isfinite(float(raw_uncertainty))
        ):
            return None
        raw_quality_flags = value.get('quality_flags', ('proxy', 'uncalibrated'))
        if not isinstance(raw_quality_flags, (list, tuple)) or not all(
            isinstance(item, str) and item for item in raw_quality_flags
        ):
            return None
        return SnowpackProxy(
            ...
            source_class=str(value.get('source_class', 'proxy')),
            source=str(value.get('source', 'open_meteo_archive')),
            uncertainty=(
            ...
            execution_status=str(value.get('execution_status', 'fallback_proxy')),
            official_warning_eligible=False,
            is_official_warning=False,
        )
...
def _proxy_from_seasonal(daily: dict) -> SnowpackProxy | None:
    ...
    return SnowpackProxy(
        ...
        quality_flags=('candidate_source', 'uncalibrated', 'heuristic_proxy'),
...
def _fallback_proxy(weather_inputs: dict[str, float], terrain_inputs: dict[str, float]) -> SnowpackProxy:
    ...
    return SnowpackProxy(
        ...
        quality_flags=('synthetic_input', 'uncalibrated', 'fallback'),
        execution_status='fallback_proxy',
        official_warning_eligible=False,
        is_official_warning=False,
    )
...
def snowpack_proxy_to_payload(proxy: SnowpackProxy | None) -> dict[str, Any] | None:
    """Serialize a proxy with explicit provenance and warning boundaries."""
    if proxy is None:
        return None
    is_synthetic = proxy.source_class == 'synthetic' or proxy.method.startswith('synthetic_')
    source_class = 'synthetic' if is_synthetic else proxy.source_class
    source = 'deterministic_fallback' if is_synthetic else proxy.source
    quality_flags = list(proxy.quality_flags)
    if is_synthetic and 'synthetic_input' not in quality_flags:
    ...
    if proxy.uncertainty is not None:
        payload['uncertainty'] = proxy.uncertainty
    if proxy.run_id:
        payload['run_id'] = proxy.run_id
    return payload
    ...
    *,
    ...
    if daily is not None:
        ...
        if proxy is not None:
            return replace(proxy, season_start=season_start.isoformat())
...
def fetch_batched_cell_snowpack_proxies_strict(
    *,
    ...
    failed = next((result for result in results if result.proxy is None), None)
    if failed is not None:
        raise RuntimeError(failed.error or 'Strict snowpack proxy fetch completed with unresolved coordinates')
    return [result.proxy for result in results if result.proxy is not None]


def fetch_batched_cell_snowpack_proxies_partial(
    *,
    coordinates: Iterable[tuple[float, float]],
    as_of: datetime,
    batch_size: int = OPEN_METEO_BATCH_SIZE,
    cache_path: Path | None = None,
) -> list[SnowpackProxyBatchResult]:
    ...
    resolved_results: list[SnowpackProxyBatchResult | None] = [None] * len(coords)
    ...
    for index, (lat, lng) in enumerate(coords):
        ...
        if cached_proxy is not None:
            resolved_results[index] = SnowpackProxyBatchResult(
                proxy=cached_proxy,
                status='ready',
            )
        else:
    ...
    for offset in range(0, len(missing_indices), effective_batch_size):
        ...
        try:
            ...
            error_text = str(exc)
            for resolved_index in batch_indices:
                resolved_results[resolved_index] = SnowpackProxyBatchResult(
                    proxy=None,
        ...
        for resolved_index, item in zip(batch_indices, items):
            ...
            if not isinstance(daily, dict):
                resolved_results[resolved_index] = SnowpackProxyBatchResult(
                    proxy=None,
                    status='unavailable_weather',
                    error=f'Missing daily seasonal weather payload for batch cell index {resolved_index}',
                )
                continue
            proxy = _proxy_from_seasonal(daily)
            if proxy is None:
                resolved_results[resolved_index] = SnowpackProxyBatchResult(
                    proxy=None,
                    status='unavailable_weather',
                    error=f'Unable to compute seasonal snowpack proxy for batch cell index {resolved_index}',
                )
                continue
            strict_proxy = replace(proxy, season_start=season_start.isoformat())
            resolved_results[resolved_index] = SnowpackProxyBatchResult(
                proxy=strict_proxy,
                status='ready',
            )
    ...
    for index, result in enumerate(resolved_results):
        if result is None:
            resolved_results[index] = SnowpackProxyBatchResult(
                proxy=None,
                status='unavailable_weather',
                error='Strict snowpack proxy fetch completed with unresolved coordinates',
            )
...
def compute_region_snowpack_proxy(
    *,
    ...
    return replace(fallback, season_start=season_start.isoformat())
