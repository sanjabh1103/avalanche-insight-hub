...
GNN_RUNOUT_ENABLED = os.getenv('GNN_RUNOUT_ENABLED', 'false').lower() in ('1', 'true', 'yes')
GNN_RUNOUT_EXTERNAL_CALIBRATED = os.getenv('GNN_RUNOUT_EXTERNAL_CALIBRATED', 'false').lower() in ('1', 'true', 'yes')
GNN_RUNOUT_HELD_OUT_VALIDATED = os.getenv('GNN_RUNOUT_HELD_OUT_VALIDATED', 'false').lower() in ('1', 'true', 'yes')
GNN_RUNOUT_PROMOTION_GATE_PASSED = os.getenv('GNN_RUNOUT_PROMOTION_GATE_PASSED', 'false').lower() in ('1', 'true', 'yes')


def _gnn_runout_promotion_status():
    return evaluate_shadow_promotion(
        'GNN_RUNOUT',
        feature_enabled=GNN_RUNOUT_ENABLED,
        external_calibrated=GNN_RUNOUT_EXTERNAL_CALIBRATED,
        held_out_validated=GNN_RUNOUT_HELD_OUT_VALIDATED,
        promotion_gate_passed=GNN_RUNOUT_PROMOTION_GATE_PASSED,
    )

# Lazy GNN engine — loaded on first use to avoid hard torch dependency
_gnn_engine: Optional[object] = None
_gnn_engine_initialized = False


def _get_gnn_engine():
    """Lazy-load GNN runout engine singleton."""
    global _gnn_engine, _gnn_engine_initialized
    if _gnn_engine_initialized:
        return _gnn_engine
    _gnn_engine_initialized = True
    if not _gnn_runout_promotion_status().active:
        return None
    try:
        from backend.models.gnn_runout import GNNRunoutEngine
        _gnn_engine = GNNRunoutEngine.from_env()
    except Exception:
        _gnn_engine = None
    return _gnn_engine
...
def _try_gnn_runout(
    *,
    ...
    """Attempt GNN runout if enabled and weights available.

    Returns a polygon [lng, lat] list if successful, None to fall back.
    """
    engine = _get_gnn_engine()
    if engine is None or not engine.is_available():
        return None
    dem_path = DEM_ROOT / f'{region_key}.tif'
    if not dem_path.exists() or not _HAS_RASTERIO:
        return None
    try:  # pragma: no cover - runtime-only when GNN weights + DEM present
        with rasterio.open(dem_path) as src:
            x_res = abs(float(src.res[0]))
            y_res = abs(float(src.res[1]))
            if src.crs and getattr(src.crs, 'is_geographic', False):
        ...
        if result is not None and result.polygon and len(result.polygon) >= 4:
...
def runout_polygon_for_cell(
    *,
    ...
    Method precedence:
        0. gnn_runout          (requires calibration/held-out/promotion gates + weights + DEM)
    ...
    # Tier 0: GNN runout (F8) — only if enabled and weights loaded
    if _gnn_runout_promotion_status().active:
        gnn_poly = _try_gnn_runout(
            region_key=region_key, lat=lat, lng=lng, probability=probability,
        )
        if gnn_poly is not None:
