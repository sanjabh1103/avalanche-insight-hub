...
OPEN_METEO_TIMEOUT = float(os.getenv('OPEN_METEO_TIMEOUT', '30.0'))
...
def _fetch_open_meteo(url: str, *, params: dict[str, Any], retries: int = 5) -> dict[str, Any]:
    ...
    for attempt in range(retries):
        ...
        if attempt < retries - 1:
            ...
            # when multiple regions retry simultaneously against the same API.
            delay = max(2 ** attempt, retry_after) + random.uniform(0, 1.5)
            time.sleep(delay)
...
BATCH_WEATHER_FETCH_ENABLED = os.getenv('BATCH_WEATHER_FETCH_ENABLED', 'false').strip().lower() in ('1', 'true', 'yes')
BATCH_WEATHER_CHUNK_SIZE = int(os.getenv('BATCH_WEATHER_CHUNK_SIZE', '50'))


def fetch_batch_weather_profile(
    cell_coords: list[tuple[float, float]],
    forecast_start: datetime,
    horizon_hours: int,
) -> list[dict[str, Any]]:
    """Fetch weather profiles for multiple coordinates using Open-Meteo batch API.

    Uses comma-separated latitude/longitude to fetch weather for multiple
    grid cells in a single API call. Falls back to per-coordinate fetch
    on error. Uses cell_selection=nearest to get the actual model grid-cell
    weather rather than elevation-matched downscaling.

    Args:
        cell_coords: List of (lat, lng) tuples for grid cell centers
        forecast_start: Forecast start datetime
        horizon_hours: Forecast horizon in hours

    Returns:
        List of weather profile dicts, one per coordinate (same order)
    """
    start = _to_utc(forecast_start)
    forecast_days = max(1, math.ceil(horizon_hours / 24.0))
    profiles: list[dict[str, Any] | None] = [None] * len(cell_coords)

    for chunk_start in range(0, len(cell_coords), BATCH_WEATHER_CHUNK_SIZE):
        chunk = cell_coords[chunk_start:chunk_start + BATCH_WEATHER_CHUNK_SIZE]
        lats = ','.join(f'{lat:.4f}' for lat, _ in chunk)
        lngs = ','.join(f'{lng:.4f}' for _, lng in chunk)
        try:
            payload = _fetch_open_meteo(
                OPEN_METEO_FORECAST,
                params={
            ...
            if isinstance(payload, list):
                for idx, item in enumerate(payload):
                    ...
                    profiles[global_idx] = {
                        'source': 'open_meteo_forecast_batch_v1',
                        'latitude': cell_coords[global_idx][0],
                        'longitude': cell_coords[global_idx][1],
                        'samples': _hourly_payload_to_samples(item),
                    }
            elif isinstance(payload, dict):
                profiles[chunk_start] = {
                    'source': 'open_meteo_forecast_batch_v1',
                    'latitude': chunk[0][0],
                    'longitude': chunk[0][1],
                    'samples': _hourly_payload_to_samples(payload),
            ...
            for idx, (lat, lng) in enumerate(chunk):
                global_idx = chunk_start + idx
                try:
                    profiles[global_idx] = fetch_forecast_weather_profile(
                        (lat, lng), forecast_start, horizon_hours,
                    )
                except Exception:
                    profiles[global_idx] = None

    for idx, p in enumerate(profiles):
        if p is None:
            profiles[idx] = fetch_forecast_weather_profile(
                cell_coords[idx], forecast_start, horizon_hours,
            )
    return profiles
...
def select_hourly_weather_sample(profile: dict[str, Any], target: datetime) -> dict[str, Any]:
    sample = profile.get('sample')
    if sample is None:
        sample = _nearest_sample(profile.get('samples') or [], _to_utc(target))
    if sample is None:
        return {}
    values = dict(getattr(sample, 'values', None) or {})
    sample_timestamp = getattr(sample, 'timestamp', None) or getattr(sample, 'time', None)
    # Preserve the source timestamp and profile lineage for the Partner input
    # contract. Feature builders ignore these metadata keys, while validation
    # and replay can now prove which hourly sample was selected.
    if sample_timestamp is not None:
        values.setdefault('time', str(sample_timestamp))
    if profile.get('source') is not None:
        values.setdefault('source_id', str(profile['source']))
    if profile.get('retrieval_time') is not None:
        values.setdefault('retrieval_time', str(profile['retrieval_time']))
    return values


def compute_dynamic_lapse_profile(
    profile: dict[str, float],
    terrain_elevation_m: float,
    fallback_lapse_rate: float | None = None,
) -> dict[str, Any]:
    base_lapse = fallback_lapse_rate if fallback_lapse_rate is not None else STANDARD_LAPSE_RATE_C_PER_M
...
def build_real_feature_row(
    *,
    ...
    snowpack_physics_override: SnowpackPhysicsResult | None = None,
    ...
    # Physics-derived features (from COSIPY/SNOWPACK) when available
    physics = snowpack_physics_override
    ...
    # Override with physics-derived values when available
    if physics is not None:
        shear_strength_norm = _clamp(physics.weak_layer_shear_strength_kpa / 12.0, 0.0, 1.0)
        settlement_rate_norm = _clamp(1.0 - physics.bulk_density_kgm3 / 600.0, 0.0, 1.0)
    ...
    feature_row = {
        ...
        # Physics-derived snowpack stratigraphy features (0.0 when unavailable)
        'weak_layer_depth': _clamp(physics.weak_layer_depth_m / 3.0, 0.0, 1.0) if physics else 0.0,
        'grain_type_faceted': 1.0 if physics and physics.weak_layer_grain_type == 'faceted' else 0.0,
        'grain_type_depth_hoar': 1.0 if physics and physics.weak_layer_grain_type == 'depth_hoar' else 0.0,
        'stability_index': _clamp(physics.snowpack_stability_index / 5.0, 0.0, 1.0) if physics else 0.5,
        'temp_gradient_profile': _clamp(physics.temperature_gradient_per_m / 0.3, 0.0, 1.0) if physics else 0.0,
        'liquid_water_content': _clamp(physics.liquid_water_content_pct / 10.0, 0.0, 1.0) if physics else 0.0,
    ...
    raw_inputs = {
        ...
        'snowpack_physics_method': physics.method if physics else 'unavailable',
        'weak_layer_grain_type': physics.weak_layer_grain_type if physics else 'unknown',
        'snowpack_stability_index': physics.snowpack_stability_index if physics else None,
        'snow_height_m': physics.snow_height_m if physics else None,
        'bulk_density_kgm3': physics.bulk_density_kgm3 if physics else None,
    }
    ...
    return {
        ...
        'snowpack_physics': physics,
    }


def build_edmrg_feature_row(
    *,
    edmrg_weather_sample: dict[str, float],
    terrain: dict[str, float],
    timestamp: datetime,
    lat: float,
    lng: float,
    ...
    """F20: Build a feature row from eDMRG telemetry instead of Open-Meteo.

    Wraps ``build_real_feature_row`` with an eDMRG-derived weather sample.
    The weather sample must already be mapped to pipeline keys by
    ``edmrg_adapter.edmrg_to_weather_samples``.
    """
    return build_real_feature_row(
        weather_sample=edmrg_weather_sample,
        terrain=terrain,
        timestamp=timestamp,
        lat=lat,
        lng=lng,
        snowpack_proxy_override=snowpack_proxy_override,
        snowpack_physics_override=snowpack_physics_override,
