...
SOURCE_WEIGHTS: dict[str, float] = {
    ...
    # F19: Auto-generated label sources
    'sar_detection': 0.85,
    'seismic_event': 0.70,
    'auto_field_report': 0.95,
}

# F19: Auto-label confidence thresholds
AUTO_LABEL_MIN_CONFIDENCE = 0.45
AUTO_LABEL_SOURCES = {'sar_detection', 'seismic_event', 'auto_field_report'}
...
def is_auto_label_eligible(record: dict[str, Any]) -> bool:
    """F19: Check if an auto-generated label meets training eligibility.

    Args:
        record: Label record with source, confidence, and governance fields

    Returns:
        True if auto-label is eligible for training
    """
    source = str(record.get('source') or '').strip().lower()
    if source not in AUTO_LABEL_SOURCES:
        return True  # Not an auto-label, use normal eligibility

    confidence = _coerce_numeric(record.get('confidence'))
    if confidence is None:
        return False

    return confidence >= AUTO_LABEL_MIN_CONFIDENCE
