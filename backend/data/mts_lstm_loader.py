...
from backend.common.sequence_features import STATIC_SEQUENCE_FEATURES, build_training_branch_arrays, extract_zone_onehot
...
class MTSAvalancheDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        self.zone_onehot = (
            np.asarray(zone_onehot, dtype=np.float32)
            if zone_onehot is not None
            else np.zeros((len(self.labels), 4), dtype=np.float32)
        )
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            'zone_onehot': torch.tensor(self.zone_onehot[idx], dtype=torch.float32),
...
def build_mts_lstm_dataset(
    ...
    *,
    ...
    # F2: Extract zone one-hot from frame's zone_type column
    zone_types = frame.get('zone_type', pd.Series(None, index=frame.index))
    zone_onehot = np.asarray(
        [extract_zone_onehot(zt if isinstance(zt, str) else None) for zt in zone_types],
        dtype=np.float32,
    ) if 'zone_type' in frame.columns else np.zeros((len(labels), 4), dtype=np.float32)
...
def build_mts_lstm_dataloaders(
    *,
    ...
    train_dataset = MTSAvalancheDataset(
        ...
        zone_onehot=train_raw_dataset.zone_onehot,
    ...
    validation_dataset = MTSAvalancheDataset(
        ...
        zone_onehot=validation_raw_dataset.zone_onehot,
        hourly_mean=stats.hourly_mean,
        hourly_std=stats.hourly_std,
        daily_mean=stats.daily_mean,
        daily_std=stats.daily_std,
