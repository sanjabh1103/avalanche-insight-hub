...
from backend.common.sar_model_family import SAR_DEFAULT_INPUT_CHANNELS, build_model_architecture, normalize_model_family
...
def train_sar_unet(
    ...
    *,
    ...
    try:
        ...
        six_channel = bool(request.get('six_channel') or int(request.get('input_channels') or SAR_DEFAULT_INPUT_CHANNELS) == 6)
        train_dataset = SarPatchDataset(dataset_root, split='train', augment=True, six_channel=six_channel)
        _write_train_status(
            artifact_dir=artifact_dir,
        ...
        normalization = compute_sar_normalization(train_dataset)
        val_dataset = SarPatchDataset(dataset_root, split='val', normalization=normalization, augment=False, six_channel=six_channel)
        ...
        model = build_model_architecture(config.model_family, image_size=config.patch_size, in_channels=6 if six_channel else 2).to(device)
