...
def run_train_mtslstm(
    ...
    *,
    ...
    snapshot_manifest = str(
        request.get('snapshot_manifest')
        or env.get('SNAPSHOT_MANIFEST')
        or ''
    ).strip()
    training_region_keys = str(
        request.get('training_region_keys')
        or env.get('TRAINING_REGION_KEYS')
        or ''
    ).strip()
    open_source_snapshot = str(
        request.get('open_source_label_snapshot')
        or env.get('OPEN_SOURCE_LABEL_SNAPSHOT')
        or ''
    ).strip()
    open_source_key = str(
        request.get('open_source_label_source_key')
        or env.get('OPEN_SOURCE_LABEL_SOURCE_KEY')
        or ''
    ).strip()
    open_source_role = str(
        request.get('open_source_label_role')
        or env.get('OPEN_SOURCE_LABEL_ROLE')
        or ''
    ).strip()
    open_source_license_review_id = str(
        request.get('open_source_label_license_review_id')
        or env.get('OPEN_SOURCE_LABEL_LICENSE_REVIEW_ID')
        or ''
    ).strip()
    env.update({
        ...
        'OPEN_SOURCE_LABEL_SNAPSHOT': open_source_snapshot,
        'OPEN_SOURCE_LABEL_SOURCE_KEY': open_source_key,
        'OPEN_SOURCE_LABEL_ROLE': open_source_role,
        'OPEN_SOURCE_LABEL_LICENSE_REVIEW_ID': open_source_license_review_id,
        'TRAINING_PREFLIGHT_STRICT': 'true',
        'TRAINING_RESEARCH_OVERRIDE': 'false',
        'ALLOW_DRIFT_SKIP': 'false',
