...
class LSTMHead:
    ...
    def _predict_stochastic_outputs(
        ...
        *,
        ...
        zone_onehot: np.ndarray | None = None,
    ) -> np.ndarray:
        ...
        zone_tensor = (
            torch.tensor(zone_onehot, dtype=torch.float32)
        ...
        with torch.no_grad():
            for sample_idx in range(samples):
                ...
                if samples > 1:
                    ...
                    self.model.eval()
                logits = self.model(hourly_tensor, daily_tensor, static_tensor, zone_onehot=zone_tensor).detach().cpu().numpy()
    ...
    def predict_sequence(
        ...
        *,
        ...
        stacked = self._predict_stochastic_outputs(branches, samples=samples, zone_onehot=zone_onehot)
...
class PlattCalibrator:
    """Wraps LogisticRegression to match the 1D .predict() interface of IsotonicRegression."""

    def __init__(self, lr: Any) -> None:
        self._lr = lr

    def predict(self, probabilities: np.ndarray) -> np.ndarray:
        proba = self._lr.predict_proba(np.asarray(probabilities, dtype=np.float32).reshape(-1, 1))
        return proba[:, 1]
...
def fit_isotonic_probability_calibrator(
    ...
    if labels_arr.size < 2 or len(np.unique(labels_arr)) < 2:
        return None, {
            'calibration_applied': False,
    ...
    def _fit_platt() -> tuple[Any | None, dict[str, Any]]:
        try:
            from sklearn.linear_model import LogisticRegression

            lr = LogisticRegression(C=1e6, solver='lbfgs', max_iter=1000)
            lr.fit(raw_prob_arr.reshape(-1, 1), labels_arr)
            return PlattCalibrator(lr), {
                'calibration_applied': True,
                'calibration_method': 'platt_sigmoid',
                'calibration_reason': 'small_sample_platt_fallback',
            }
        except Exception as exc:
            return None, {
                'calibration_applied': False,
                'calibration_method': 'unavailable',
                'calibration_reason': f'platt_fallback_failed: {exc}',
            }

    if labels_arr.size < MTS_MIN_CALIBRATION_ROWS:
        return _fit_platt()
    try:
    ...
    *,
    ...
    for epoch in range(epochs_requested):
        for batch in train_loader:
            optimizer.zero_grad()
            logits = model(batch['hourly'], batch['daily'], batch['static'], zone_onehot=batch.get('zone_onehot'))
        ...
        with torch.no_grad():
            for batch in validation_loader:
                validation_logits = model(batch['hourly'], batch['daily'], batch['static'], zone_onehot=batch.get('zone_onehot'))
