...
def _poll_until_terminal(
    *,
    ...
    while True:
        ...
        # Defense-in-depth: reject non-terminal body statuses even with HTTP 200.
        body_status = str(body.get('status') or '').strip().lower()
        if is_non_terminal(body_status):
            print(f'train-sar-unet: body status "{body_status}" is non-terminal, continuing to poll', file=sys.stderr)
            time.sleep(max(int(poll_interval_seconds), 1))
            continue
        return body
