...
from backend.common.verification_contracts import VerificationPacket
from backend.common.release_policy import evaluate_release_decision, is_research_gate_enabled
...
def activate_dynamic_model_candidate(
    *,
    ...
    if execute_activation and not blockers and not already_active:
        review_decision = evaluate_scientist_review_gate(
            'model_activation',
            VerificationPacket(
                cell_id=f'model:{candidate_version}',
                region_key=str(candidate.get('region_key') or 'global'),
                lineage={'candidate_version': candidate_version},
            ),
            gate_key='mts_lstm_promotion_gate',
        )
        if not review_decision.approved:
            blockers.append('scientist_review')
            result['blockers'] = blockers
            result['action'] = 'blocked'
            result['scientist_review'] = review_decision.to_dict()
            return result
    if execute_activation:
        _gate_enabled = is_research_gate_enabled()
        _gates = _as_dict(candidate.get('gates'))
        _publication_gates_passed = all(
            bool(_gates.get(g))
            for g in (
                'shadow_quality_gate_passed',
                'sar_release_gate_passed',
                'sar_volume_gate_passed',
                'production_eligibility_gate_passed',
            )
        )
        _release_decision = evaluate_release_decision(
            candidate_type,
            candidate_version,
            gate_enabled=_gate_enabled,
            publication_gates_passed=_publication_gates_passed,
        )
        result['release_decision'] = _release_decision.as_dict()
        if not _release_decision.allowed:
            blockers.append('release_policy')
            result['blockers'] = blockers
            result['action'] = 'blocked'
            return result
    ...
    patch_latest_model_status_row(
        {
            ...
            'release_decision': _release_decision.as_dict(),
        },
    )
