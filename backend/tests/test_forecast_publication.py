...
class ForecastPublicationTests(unittest.TestCase):
    ...
    def test_promote_forecast_run_returns_promoted_row(self, rest_rpc_mock, _record_event_mock) -> None:
        ...
        promoted = promote_forecast_run(forecast_run_id='run-1', model_type='surrogate_rf_v1', model_version='v1', publication_gates_passed=True)
    ...
    def test_promote_forecast_run_blocked_by_research_gate(self) -> None:
        with self.assertRaises(RuntimeError) as ctx:
            promote_forecast_run(forecast_run_id='run-1', model_type='mts_lstm_v1', model_version='v2', publication_gates_passed=True)
        self.assertIn('research_gate_non_baseline', str(ctx.exception))

    @patch('backend.common.forecast_publication._record_event')
    @patch('backend.common.forecast_publication.rest_rpc')
    @patch.dict('os.environ', {'RESEARCH_MODEL_GATE_ENABLED': 'true'})
    def test_promote_forecast_run_baseline_technical_artifact(self, rest_rpc_mock, _record_event_mock) -> None:
        rest_rpc_mock.return_value = {
            'id': 'run-1',
            'published_at': '2026-05-08T02:00:00+00:00',
            'active': True,
        ...
        self.assertEqual(promoted['id'], 'run-1')
        detail = _record_event_mock.call_args.kwargs['detail']
        self.assertEqual(detail['artifact_mode'], 'technical_artifact')
        self.assertEqual(detail['warning_authority'], 'none')


    @patch.dict('os.environ', {'RESEARCH_MODEL_GATE_ENABLED': 'false'})
    def test_promote_forecast_run_requires_all_args(self) -> None:
        """Regression: promote_forecast_run must not have permissive defaults."""
        with self.assertRaises(TypeError):
            promote_forecast_run(forecast_run_id='run-1')  # type: ignore[call-arg]
