...
class PublicationProofTests(unittest.TestCase):
    def test_parse_forecast_start_accepts_aware_rfc3339_timestamp(self) -> None:
        parsed = _parse_forecast_start('2026-07-05T00:00:00Z')
        self.assertEqual(parsed, pd.Timestamp('2026-07-05T00:00:00Z'))
...
class ForecastGridMetadataTests(unittest.TestCase):
    def setUp(self) -> None:
        # Forecast publication appends calibration evidence. Keep fixture
        # runs from mutating the repository's default history file and thereby
        # changing the release-candidate scope during tests.
        self._calibration_history_patcher = patch.dict(
            os.environ,
            {'CALIBRATION_HISTORY_PATH': ''},
            clear=False,
        )
        self._calibration_history_patcher.start()
        from backend.common.uncertainty_quantification import UQResult
        _uq = UQResult(brier_score=None, forecast_confidence='unknown', publish_blocked=False, block_reason=None, conformal_alpha=0.1)
        self._uq_patcher = patch(
            'backend.daily_inference.apply_uq_to_cells',
            side_effect=lambda rows, meta, calibrator=None: (rows, _uq),
        )
        self._uq_patcher.start()
        self._seismic_patcher = patch(
            'backend.daily_inference.fetch_recent_earthquakes',
            return_value=[],
        )
        self._seismic_patcher.start()
        self._sachet_rss_patcher = patch(
            'backend.daily_inference.ingest_sachet_alerts',
            return_value=([], None),
        )
        self._sachet_rss_patcher.start()
        self._scientist_gate_patcher = patch(
            'backend.common.scientist_review_gate._check_scientist_validation_case',
            return_value='test-case-id',
        )
        self._scientist_gate_patcher.start()
        # G-01/G-15: Mock evaluate_publication_evidence to return allowed decision
        # so promote_forecast_run is called in tests
        from backend.common.release_policy import ReleaseDecision
        _allowed_decision = ReleaseDecision(
            allowed=True,
        ...
        self._pub_evidence_patcher = patch(
            'backend.daily_inference.evaluate_publication_evidence',
            return_value=_allowed_decision,
        )
        self._pub_evidence_patcher.start()

    def tearDown(self) -> None:
        self._uq_patcher.stop()
        self._seismic_patcher.stop()
        self._sachet_rss_patcher.stop()
        self._scientist_gate_patcher.stop()
        self._pub_evidence_patcher.stop()
        self._calibration_history_patcher.stop()
    ...
    def test_upsert_forecast_grid_proof_mode_skips_compatibility_and_emits_stage_event(
        ...
        promote_forecast_run_mock.assert_called_once_with(
            forecast_run_id='run-proof-1',
            model_type='surrogate_rf_v1',
            model_version='unknown',
    ...
    @patch('backend.daily_inference.RUN_PHYSICS_RUNOUT', False)
    @patch('backend.daily_inference.has_supabase_credentials', return_value=False)
    @patch('backend.daily_inference._fetch_region_sar_evidence', return_value={'mask_asset_refs': [], 'sar_event_geometries': []})
    ...
    def test_approximate_tree_shap_is_explicit_in_proof_metadata(self) -> None:
        options = ProofModeOptions(approximate_tree_shap=True)

        self.assertTrue(options.approximate_tree_shap)
        self.assertTrue(options.as_metadata()['approximate_tree_shap'])
    ...
    def test_build_cells_preserves_surrogate_explanation_contract(
        ...
        self.assertIsInstance(cell['risk_score'], (int, float))
        self.assertIsInstance(cell['probability_risk_score'], (int, float))
    ...
    def test_build_cells_marks_unavailable_terrain_cells_stale(
        ...
        self.assertEqual(cell['terrain_failure_reason'], 'terrain_value_error')
        self.assertEqual(cell['terrain_inputs']['failure_reason'], 'terrain_value_error')
        self.assertFalse(cell['apt_eligible'])
        self.assertEqual(cell['apt_profile'], 'apt_30_50_v1')
        self.assertEqual(cell['apt_mask_reason'], 'slope_outside_30_to_50_deg')
        self.assertEqual(cell['risk_score'], 0)
...
class PublicEligibilityAndClassifierTests(unittest.TestCase):
    ...
    def test_problem_classifier_rain_on_snow_with_fresh_snowfall_but_no_depth(self) -> None:
        """Regression: rain-on-snow proxy should trigger wet_snow when
        snowfall_24h >= 5cm even if snow_depth < 10cm and no seasonal support.
        Previously _rain_on_snow_proxy() omitted the snowfall_24h condition."""
        problem = classify_avalanche_problem(
            weather_inputs={
                'downscaled_temperature_c': 2.0,
                'snowfall_24h_cm': 6.0,
                'precipitation_24h_mm': 8.0,
                'snow_depth_cm': 5.0,
                'wind_loading': 0.1,
            ...
            terrain_inputs={'aspect_loading': 0.2, 'aspect_deg': 180.0},
            snowpack_proxy={'method': 'proxy_v1'},
            forecast_time=datetime(2026, 4, 25, 3, 0, 0),
            timezone_name='UTC',
        )

        self.assertEqual(problem['problem_slug'], 'wet_snow')
        self.assertIn('rain_on_snow_proxy', problem['problem_evidence'])


class NetworkIsolationTests(unittest.TestCase):
    """Regression: prove no live HTTP requests occur during test execution."""

    def test_build_publication_proof_no_network_with_empty_outputs(self):
        """build_publication_proof must not make network calls when supabase_enabled=False."""
        proof = build_publication_proof(
            outputs=[],
            generated_at=datetime(2026, 5, 8, 4, 0, tzinfo=timezone.utc),
            dry_run=True,
            ...
            expected_forecast_date='2026-05-08',
            artifact_dir=Path('/tmp/artifacts/test'),
        )
