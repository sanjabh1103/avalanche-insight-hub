...
class OpenMeteoTests(unittest.TestCase):
    ...
    def test_fetch_historical_weather_window_falls_back_to_archive_after_rate_limits(self) -> None:
        ...
        with patch.object(
            real_features.requests,
            'get',
            side_effect=[transient, transient, transient, transient, transient, archive_success],
        ) as get_mock:
            with patch.object(real_features.time, 'sleep', return_value=None) as sleep_mock:
...
class BatchWeatherFetchTests(unittest.TestCase):
    def test_batch_fetch_returns_profiles_for_multiple_coords(self) -> None:
        multi_payload = [
            {'hourly': {'time': ['2024-01-01T00:00', '2024-01-01T01:00'], 'temperature_2m': [-5.0, -4.0]}},
            {'hourly': {'time': ['2024-01-01T00:00', '2024-01-01T01:00'], 'temperature_2m': [-8.0, -7.0]}},
        ]
        with patch.object(real_features, '_fetch_open_meteo', return_value=multi_payload):
            profiles = fetch_batch_weather_profile(
                [(46.0, 8.0), (46.5, 8.5)],
                datetime(2024, 1, 1, tzinfo=timezone.utc),
                72,
            )
        self.assertEqual(len(profiles), 2)
        self.assertEqual(profiles[0]['source'], 'open_meteo_forecast_batch_v1')
        self.assertEqual(len(profiles[0]['samples']), 2)
        self.assertAlmostEqual(profiles[0]['samples'][0].values['temperature_2m'], -5.0)
        self.assertAlmostEqual(profiles[1]['samples'][0].values['temperature_2m'], -8.0)

    def test_batch_fetch_falls_back_to_single_on_error(self) -> None:
        single_payload = {'hourly': {'time': ['2024-01-01T00:00'], 'temperature_2m': [-3.0]}}
        with patch.object(real_features, '_fetch_open_meteo', side_effect=RuntimeError('API down')):
            with patch.object(real_features, 'fetch_forecast_weather_profile', return_value={
        ...
        self.assertEqual(profiles[0]['source'], 'open_meteo_forecast_downscaled_v1')
