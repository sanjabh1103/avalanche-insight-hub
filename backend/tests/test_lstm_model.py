...
class _DummyHead:
    ...
    def predict_sequence(self, branches: SequenceBranches, *, mc_samples: int | None = None, zone_onehot: np.ndarray | None = None):
...
class PredictProductionProbabilityTests(unittest.TestCase):
    ...
    def test_predict_sequence_respects_apply_calibration_flag(self) -> None:
        ...
        head._predict_stochastic_outputs = lambda branches, samples, seed_base=None, zone_onehot=None: np.asarray(  # type: ignore[method-assign]
            [[0.2], [0.4]],
            dtype=np.float32,
        )
