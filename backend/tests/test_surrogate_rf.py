...
class SurrogateRfBundleTests(unittest.TestCase):
    ...
    def test_tree_shap_batch_forwards_approximate_mode(self) -> None:
        calls: list[dict[str, object]] = []
        selected_features = ['snowfall_24h', 'wind_loading']
        selected_frame = pd.DataFrame([[0.2, 0.4]], columns=selected_features)

        class _Explainer:
            def shap_values(self, frame: pd.DataFrame, **kwargs: object) -> np.ndarray:
                calls.append(dict(kwargs))
                return np.zeros((len(frame), len(selected_features)), dtype=float)

        packets = compute_tree_shap_batch(
            _Explainer(),
            selected_frame,
            selected_features,
            approximate=True,
        ...
        self.assertEqual(calls, [{'approximate': True}])
...
class PhysicalPlausibilityRuleTests(unittest.TestCase):
    """Tests for the 8 physical plausibility rules in physically_valid_surrogate_rows."""

    def test_rule6_temp_gradient_bounds_rejects_extreme_values(self) -> None:
        frame = pd.DataFrame({
            'temp_gradient': [0.25, 0.15, 1.5, -0.5, 0.40, np.nan],
        })
        valid = physically_valid_surrogate_rows(frame)
        self.assertTrue(valid[0])
        self.assertTrue(valid[1])
        self.assertFalse(valid[2])
        self.assertFalse(valid[3])
        self.assertTrue(valid[4])
        self.assertTrue(valid[5])

    def test_rule7_heavy_snowfall_at_warm_temp_rejected(self) -> None:
        frame = pd.DataFrame({
            'snowfall_24h': [5.0, 15.0, 3.0, 20.0, 2.0],
            'temperature_2m': [-5.0, 12.0, -3.0, 15.0, -10.0],
        })
        valid = physically_valid_surrogate_rows(frame)
        self.assertTrue(valid[0])
    ...
    def test_rule8_northness_wet_activation_rejected(self) -> None:
        frame = pd.DataFrame({
            'northness': [0.8, 0.3, 0.9, -0.5, 0.75],
            'wet_activation_signal': [0.8, 0.6, 0.9, 0.9, 0.3],
        })
        valid = physically_valid_surrogate_rows(frame)
    ...
    def test_all_rules_pass_on_valid_data(self) -> None:
        frame = pd.DataFrame({
            'temperature_2m': [-8.0, -5.0, -12.0],
            'elevation_m': [3100.0, 2800.0, 3500.0],
            'snowfall_24h': [3.0, 5.0, 1.0],
            'precipitation_24h': [2.0, 3.0, 1.0],
            'snow_settlement_index': [0.3, 0.4, 0.2],
            'temp_gradient': [0.25, 0.20, 0.35],
            'northness': [0.5, -0.3, 0.2],
            'wet_activation_signal': [0.2, 0.1, 0.3],
        })
        valid = physically_valid_surrogate_rows(frame)
