...
class ModelStatusStateTests(unittest.TestCase):
    ...
    def test_resolve_active_model_state_candidate_blocked_when_gates_not_passed(self) -> None:
        """Regression: candidate activation must not bypass publication gates.

        When publication_gates_passed=False (the default), a non-baseline
        candidate must be blocked even if it is otherwise ready for activation.
        """
        bundle = {
            'created_at': '2026-04-29T00:00:00+00:00',
            'surrogate_model_version': 'rf_surrogate_v2',
            'dynamic_model_version': 'mts-lstm-shadow-2',
            'metrics': {'pss_reported': 0.61, 'pss_gate_passed': True},
            'lstm_head_meta': {
                'enabled': True,
                'promotion_gate_passed': True,
                'shadow_quality_gate_passed': True,
        ...
        with patch.dict('os.environ', {'RESEARCH_MODEL_GATE_ENABLED': 'true'}):
            state = resolve_active_model_state(
                current_status, candidate, bundle, publication_gates_passed=False,
            )

        self.assertFalse(state['promotion_gate_passed'])
        self.assertIn('release_decision', state)
        self.assertFalse(state['release_decision']['allowed'])

    def test_resolve_active_model_state_always_has_release_decision(self) -> None:
        """Regression: resolve_active_model_state must always return a release_decision."""
        bundle = {'created_at': '2026-04-29T00:00:00+00:00', 'surrogate_model_version': 'rf_v1'}
        candidate = build_dynamic_model_candidate(bundle)
