...
class NewsIngestTests(unittest.TestCase):
    ...
    def test_post_ingest_event_includes_autonomous_label_fields(self) -> None:
        ...
        self.assertEqual(payload['location'], 'POINT(-121.0 46.0)')
        topo = payload['topo_profile']
        self.assertEqual(topo['metadata']['corroboration_sources'], ['gemini_news'])
        self.assertEqual(
            topo['metadata']['machine_candidate_reason'],
            'gemini_extracted_news_unreviewed',
        )
        self.assertEqual(topo['location_name'], 'Test Pass')
