...
describe('ScientistDailyVerificationPage', () => {
  ...
  it('renders paired scientist-vs-model verification controls for scientist role', async () => {
    ...
    expect((await screen.findAllByText(/2026-05-21 · Himalayas Nepal/i)).length).toBeGreaterThan(0);
    expect(screen.getByText(/Scientist 3/i)).toBeTruthy();
    expect(screen.getByText('Danger agreement')).toBeTruthy();
    expect(screen.getByText('50%')).toBeTruthy();
    expect(screen.getByText('Disagreements')).toBeTruthy();
    expect(screen.getByText('False-alarm candidates')).toBeTruthy();
    expect(screen.getByText('Miss candidates')).toBeTruthy();
    expect(screen.getByText('High-danger recall proxy')).toBeTruthy();
    expect(screen.getByText('Disagreement queue')).toBeTruthy();
    expect(screen.getByText(/s3\/m2/i)).toBeTruthy();
