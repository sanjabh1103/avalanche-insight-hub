...
import {
  buildScientistReviewOperationsAnalytics,
  buildScientistWeeklyReadinessReport,
} from '@/lib/scientistReadinessReport';
...
describe('scientist validation export', () => {
  ...
  it('reports review throughput and priority-five sign-off without promotion semantics', () => {
    const caseRow = {
      id: 'case-ops-1',
      priority: 5,
      status: 'reviewed',
      ...
      reviewed_at: '2026-05-28T02:00:00Z',
      disagreement_count: 1,
    } as ScientistValidationCase;
    const firstReview = makeReview('case-ops-1', 'reviewer-a', 'accepted');
    firstReview.created_at = '2026-05-28T01:00:00Z';
    const secondReview = makeReview('case-ops-1', 'reviewer-b', 'accepted');
    secondReview.created_at = '2026-05-28T01:30:00Z';

    const analytics = buildScientistReviewOperationsAnalytics(
      [caseRow],
      [firstReview, secondReview],
      new Date('2026-05-29T00:00:00Z'),
    );

    expect(analytics.reviewed_this_week).toBe(1);
    expect(analytics.priority_5_two_reviewer_signoff).toBe(1);
    expect(analytics.priority_5_first_response_hours_avg).toBe(1);
    expect(analytics.priority_5_decision_hours_avg).toBe(2);
    expect(analytics.disagreement_count).toBe(1);
    expect(analytics.claim_boundary).toContain('never auto-promote');
  });
});
