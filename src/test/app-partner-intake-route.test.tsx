...
const { partnerIntakeFlag } = vi.hoisted(() => ({
  partnerIntakeFlag: { current: false },
}));

vi.mock('@/lib/featureFlags', () => ({
  getDefaultFeatureFlags: () => ({ partnerIntake: partnerIntakeFlag.current }),
  resolveFeatureFlags: (env: { VITE_FEATURE_PARTNER_INTAKE?: string }) => ({
    partnerIntake: env.VITE_FEATURE_PARTNER_INTAKE === 'true',
  }),
}));

async function renderAppAt(path: string, partnerIntakeEnabled: boolean) {
  partnerIntakeFlag.current = partnerIntakeEnabled;
...
describe('partner-intake route feature flag', () => {
  ...
  it('does not mount the partner-intake route when the feature flag is off', async () => {
    ...
    expect(screen.getByText(/This page doesn't exist or has moved/i)).toBeTruthy();
  });
