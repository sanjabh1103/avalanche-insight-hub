...
import { type ForecastMode } from '@/lib/constants';
import { buildVerificationSpineFixture, VERIFICATION_SPINE_FIXTURE_KEY } from '@/lib/verificationSpineFixture';
...
function buildPublicMaskSmokeFixture(): {
  ...
  const row: ForecastGridRowRecord = {
    ...
    model_metadata: {
      ...
      technical_artifact_path: '/artifacts/public-mask-smoke/run_derived_artifact.json',
      technical_artifact_id: 'rda_pms_fixture_abc123def456',
      technical_artifact_sha256: 'fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210',
...
export function useForecastState() {
  ...
  const [multiHazardEnabled, setMultiHazardEnabled] = useState(false);
  ...
  const forecastMode = useMemo<ForecastMode>(() => {
    const raw = activeModelMetadata?.forecast_mode;
    if (typeof raw === 'string' && (raw === 'full' || raw === 'cold_start' || raw === 'transfer')) {
      return raw;
    }
    return 'full';
  }, [activeModelMetadata]);

  const seismicEventsActive = useMemo<boolean>(() => {
    return Boolean(activeModelMetadata?.seismic_events_active);
  }, [activeModelMetadata]);

  // F15: AAVDS events from model metadata
  const aavdsEvents = useMemo<import('@/components/AAVDSOverlay').AAVDSEventData[]>(() => {
    const aavds = activeModelMetadata?.aavds as Record<string, unknown> | undefined;
    if (!aavds || !Array.isArray(aavds.events)) return [];
    return aavds.events as import('@/components/AAVDSOverlay').AAVDSEventData[];
  }, [activeModelMetadata]);

  // F18: Citizen science reports from model metadata
  const citizenReports = useMemo<import('@/components/CitizenReportPanel').CitizenReportData[]>(() => {
    const cs = activeModelMetadata?.citizen_science as Record<string, unknown> | undefined;
    if (!cs || !Array.isArray(cs.reports)) return [];
    return cs.reports as import('@/components/CitizenReportPanel').CitizenReportData[];
  }, [activeModelMetadata]);

  // F7: Backend sensor events from model metadata (merged with manual paste)
  const backendSensorEvents = useMemo<import('@/components/SensorOverlay').SensorEvent[]>(() => {
    const se = activeModelMetadata?.sensor_events as Record<string, unknown> | undefined;
    if (!se || !Array.isArray(se.events)) return [];
    return se.events as import('@/components/SensorOverlay').SensorEvent[];
  }, [activeModelMetadata]);

  // F7: Sensor overlay state
  const [manualSensorEvents, setManualSensorEvents] = useState<import('@/components/SensorOverlay').SensorEvent[]>([]);
  const [showSensorOverlay, setShowSensorOverlay] = useState(false);

  // Merge backend-fetched events with manually pasted events
  const sensorEvents = useMemo(() => [...backendSensorEvents, ...manualSensorEvents], [backendSensorEvents, manualSensorEvents]);

  // Auto-show overlay when backend events are present
  useEffect(() => {
    if (backendSensorEvents.length > 0) setShowSensorOverlay(true);
  }, [backendSensorEvents]);

  const toggleSensorOverlay = useCallback(() => {
    setShowSensorOverlay(prev => !prev);
  }, []);

  const setSensorEvents = setManualSensorEvents;

  const ingestSensorData = useCallback((rawData: string) => {
    try {
      const parsed = JSON.parse(rawData);
      const events: import('@/components/SensorOverlay').SensorEvent[] = Array.isArray(parsed)
        ? parsed
        : parsed.events ?? parsed.data?.events ?? parsed.data ?? [];
      const valid = events.filter(
        (e: import('@/components/SensorOverlay').SensorEvent) =>
          e && typeof e.lat === 'number' && typeof e.lng === 'number' && e.event_id,
      );
      setSensorEvents(valid);
      setShowSensorOverlay(true);
      toast.success(`Ingested ${valid.length} sensor event${valid.length !== 1 ? 's' : ''}`);
    } catch {
      try {
        // Try CSV parse
        ...
        const csvEvents: import('@/components/SensorOverlay').SensorEvent[] = [];
        for (let i = 1; i < lines.length; i++) {
          const vals = lines[i].split(',');
          const row: Record<string, string> = {};
          headers.forEach((h, j) => { row[h] = (vals[j] ?? '').trim(); });
          if (row.event_id && row.lat && row.lng) {
            csvEvents.push({
              ...
              velocity_ms: row.velocity_ms ? parseFloat(row.velocity_ms) : null,
              mass_kg: row.mass_kg ? parseFloat(row.mass_kg) : null,
              depth_m: row.depth_m ? parseFloat(row.depth_m) : null,
              impact_pressure_kpa: row.impact_pressure_kpa ? parseFloat(row.impact_pressure_kpa) : null,
              rtsp_url: row.rtsp_url || null,
              image_url: row.image_url || null,
            });
          }
        }
        setSensorEvents(csvEvents);
        setShowSensorOverlay(true);
        toast.success(`Ingested ${csvEvents.length} sensor event${csvEvents.length !== 1 ? 's' : ''}`);
      } catch {
  ...
  useEffect(() => {
    ...
    if (fixtureKey === VERIFICATION_SPINE_FIXTURE_KEY) {
      const fixture = buildVerificationSpineFixture();
      setRegion(fixture.region);
      setArtifactHourRefs(null);
      const safeHourIdx = Math.max(0, Math.min(hourValue, fixture.grids.length - 1));
      hydrateForecastGridRow(fixture.row, {
        grids: fixture.grids,
        notice: 'Verification spine demo fixture loaded — anomaly badges, consensus gauges, and baseline deviations active.',
        bulletin: fixture.bulletin,
      });
      setTimeOffset(safeHourIdx);
      const cellParam = params.get('cell');
      ...
      toast.success('Loaded verification spine demo fixture');
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (alive && latest) {
          ...
          toast.success(`Loaded forecast for ${region.name}`);
