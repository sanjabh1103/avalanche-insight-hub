...
export default function Index() {
  ...
  const featureFlags = useMemo(() => resolveFeatureFlags(import.meta.env as unknown as Record<string, string>), []);
  const artifactMode = (state.activeModelMetadata?.artifact_mode as string | undefined) ?? undefined;
  const artifactPath = (state.activeModelMetadata?.technical_artifact_path as string | undefined) ?? undefined;
  const artifactError = (state.activeModelMetadata?.technical_artifact_error as string | undefined) ?? undefined;
  const _artifactAsset = (state.activeModelMetadata?.technical_artifact_asset as Record<string, unknown> | undefined) ?? undefined;
  const artifactId = (_artifactAsset?.artifact_id as string | undefined) ?? (state.activeModelMetadata?.technical_artifact_id as string | undefined) ?? undefined;
  const artifactSha256 = (_artifactAsset?.sha256 as string | undefined) ?? (state.activeModelMetadata?.technical_artifact_sha256 as string | undefined) ?? undefined;
  const runId = (state.activeForecastRow?.id as string | undefined) ?? undefined;
  const calibrationState = (state.activeModelMetadata?.calibration_lineage as Record<string, unknown> | undefined)?.state as string | undefined ?? undefined;
  const shadowLabel = (state.activeModelMetadata?.release_decision as Record<string, unknown> | undefined)?.artifact_mode === 'technical_artifact';
  ...
  return (
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-background text-foreground">
      <DisclaimerBanner />
      {state.hourlyGrids && (
        <div className="px-4 py-1 z-50 flex items-center gap-2">
          <DataLatencyBanner
          ...
          <TechnicalArtifactBadge
            ...
            technicalArtifactEnabled={featureFlags.technicalArtifact}
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <ForecastSidebar
          ...
          multiHazardEnabled={state.multiHazardEnabled}
        />
        ...
        <div className="flex-1 relative flex flex-col min-h-0">
          ...
          <ForecastTopControls
            ...
            multiHazardEnabled={state.multiHazardEnabled}
            setMultiHazardEnabled={state.setMultiHazardEnabled}
          />
            ...
            <Suspense fallback={<MapSurfaceFallback />}>
              <MapErrorBoundary>
                <LazyAvalancheMap
                  ...
                  seismicEventsActive={state.seismicEventsActive}
                  sensorEvents={state.sensorEvents}
                  showSensorOverlay={state.showSensorOverlay}
                  aavdsEvents={state.aavdsEvents}
                />
              </MapErrorBoundary>
            </Suspense>
            {!state.hourlyGrids && (
        ...
        {state.expertPanelOpen && (
          <Suspense fallback={<ExpertPanelFallback />}>
            <LazyExpertModePanel
              ...
              cells={state.grid.cells}
              gridSize={state.activeForecastRow?.grid_size ?? GRID_SIZE}
