...
import { ArrowLeft, ClipboardCheck, Download, FileCheck2, Mountain, ShieldCheck, UploadCloud } from 'lucide-react';
...
import { fetchDailyVerifications } from '@/lib/scientistValidation';
...
export default function ScientistPage() {
  ...
  const exportWeeklyReadinessReport = async () => {
    ...
    try {
      const records = await fetchDailyVerifications(100);
      downloadText('scientist-weekly-readiness-report.json', buildScientistWeeklyReadinessReportJson(records));
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col px-4 py-4 sm:px-6 lg:px-8">
        <header className="rounded-[1.75rem] border border-border/70 bg-card/70 px-4 py-4 shadow-2xl shadow-black/20 backdrop-blur-2xl">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className={`grid grid-cols-1 gap-2 ${partnerIntakeEnabled ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}>
