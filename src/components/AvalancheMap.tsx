...
const LazySeismicOverlay = lazy(() => import('@/components/SeismicOverlay'));
const LazySensorOverlay = lazy(() => import('@/components/SensorOverlay'));
const LazyAAVDSOverlay = lazy(() => import('@/components/AAVDSOverlay').then(m => ({ default: m.AAVDSOverlay })));
import type { SensorEvent as SensorEventType } from '@/components/SensorOverlay';
import type { AAVDSEventData } from '@/components/AAVDSOverlay';
...
interface Props {
  ...
  seismicEventsActive?: boolean;
  sensorEvents?: SensorEventType[];
  showSensorOverlay?: boolean;
  aavdsEvents?: AAVDSEventData[];
}
...
export default function AvalancheMap({
  ...
  return (
    <MapContainer center={center} zoom={zoom} className="h-full w-full z-0 theme-aware-map" zoomControl={true} touchZoom={true} dragging={true}>
      ...
      {/* Seismic cascade overlay (F1) */}
      {seismicEventsActive ? (
        <Suspense fallback={<OverlayLoadingNotice label="Loading seismic overlay" />}>
          <LazySeismicOverlay cells={cells} visible={true} />
        </Suspense>
      ) : null}

      {/* Ground radar / sensor overlay (F7) */}
      {showSensorOverlay && sensorEvents && sensorEvents.length > 0 ? (
        <Suspense fallback={<OverlayLoadingNotice label="Loading sensor overlay" />}>
          <LazySensorOverlay events={sensorEvents} visible={true} />
        </Suspense>
      ) : null}

      {/* AAVDS victim detection overlay (F15) */}
      {aavdsEvents && aavdsEvents.length > 0 ? (
        <Suspense fallback={<OverlayLoadingNotice label="Loading AAVDS overlay" />}>
          <LazyAAVDSOverlay events={aavdsEvents} />
        </Suspense>
      ) : null}
    </MapContainer>
  );
