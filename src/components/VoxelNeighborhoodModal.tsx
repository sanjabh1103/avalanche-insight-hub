...
function VoxelScene({
  ...
  // Update colors (reactively to cells changing via timeline scrub or view mode toggle)
  useEffect(() => {
    if (!meshRef.current) return;
    try {
      voxels.forEach((v, i) => {
        ...
        if (cell) {
          if (isCellUnavailable(cell)) {
            ...
            const anomalyState = cell.verificationPacket?.anomaly_state ?? 'unverified';
            const anomalyC = new THREE.Color(ANOMALY_COLORS[anomalyState] ?? ANOMALY_COLORS.unverified);
            baseC.lerp(anomalyC, 0.8);
  ...
  }, [cells, viewMode, onRenderError, onRenderReady, voxels]);
  ...
  const handlePointerMove = useCallback((e: ThreeEvent<PointerEvent>) => {
    ...
    if (e.instanceId !== undefined && e.instanceId !== null) {
      ...
      if (v) {
        ...
        const anomalyState = cell?.verificationPacket?.anomaly_state ?? null;
        const zscore = cell?.verificationPacket?.residual_zscore ?? null;
        const attribution = cell?.verificationPacket?.attribution_bucket ?? null;
        const hoverColor = unavailable || masked
          ? '#6b7280'
          : viewMode === 'anomaly'
            ? (ANOMALY_COLORS[anomalyState ?? 'unverified'] ?? '#333')
            : isHighUncertainty(cell) ? '#9ca3af' : (RISK_COLORS[score] || '#333');
        onHover({
...
function VoxelFallbackCanvas({
  ...
  useEffect(() => {
    ...
    for (const voxel of voxels.slice(0, 2500)) {
      ...
      const anomalyState = cell?.verificationPacket?.anomaly_state ?? 'unverified';
      ctx.fillStyle = isCellUnavailable(cell) || isCellMasked(cell)
        ? '#6b7280'
        : viewMode === 'anomaly'
          ? (ANOMALY_COLORS[anomalyState] ?? '#334155')
          : isHighUncertainty(cell) ? '#9ca3af' : (RISK_COLORS[riskScore] || '#334155');
  ...
  }, [voxels, cells, viewMode, canvasBg]);
...
export default function VoxelNeighborhoodModal({ open, onClose, bbox, gridCells, hourlyGrids, timeOffset }: Props) {
  ...
  const [viewMode, setViewMode] = useState<'risk' | 'anomaly'>('risk');
      ...
      >
        <DialogHeader className="p-4 pb-2 border-b border-border shrink-0">
          <DialogTitle className="flex items-center gap-2 text-foreground text-base">
            <span className="text-lg">🧊</span> 3D Neighborhood {viewMode === 'anomaly' ? 'Anomaly' : 'Risk'} Map
            <div className="ml-auto flex items-center gap-1 rounded-lg border border-border bg-card/50 p-0.5">
              <button
                onClick={() => setViewMode('risk')}
                className={`px-2.5 py-1 text-[10px] font-medium rounded-md transition-colors ${viewMode === 'risk' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >Risk View</button>
              <button
                onClick={() => setViewMode('anomaly')}
                className={`px-2.5 py-1 text-[10px] font-medium rounded-md transition-colors ${viewMode === 'anomaly' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >Anomaly View</button>
            </div>
          </DialogTitle>
        ...
        <div className="flex-1 relative min-h-0" style={{ background: canvasBg }}>
          {loading ? (
            ...
            <VoxelFallbackCanvas voxels={renderedBlocks!} cells={currentCells} viewMode={viewMode} canvasBg={canvasBg} />
          ) : (
            <div 
          ...
          {hovered && (
            <div className="absolute top-3 left-3 z-20 glass-panel rounded-lg p-3 space-y-1.5 min-w-[200px] pointer-events-none">
              ...
              <div className="text-[10px] text-muted-foreground space-y-0.5 mt-2">
                ...
                {viewMode === 'anomaly' && hovered.anomalyState && (
                  <>
                    <div><strong>Anomaly:</strong> {ANOMALY_LABELS[hovered.anomalyState] ?? hovered.anomalyState}</div>
                    {hovered.zscore !== null && <div><strong>Z-score:</strong> {hovered.zscore.toFixed(2)}</div>}
                    {hovered.attribution && <div><strong>Attribution:</strong> {hovered.attribution}</div>}
                  </>
                )}
          ...
          <div className="absolute bottom-3 left-3 z-10 glass-panel rounded-lg p-2 space-y-1">
            {viewMode === 'anomaly' ? (
              <>
                {(['anomaly', 'watch', 'normal', 'unverified'] as const).map(state => (
                  <div key={state} className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: ANOMALY_COLORS[state] }} />
                    <span className="text-[10px] text-muted-foreground">{ANOMALY_LABELS[state]}</span>
                  </div>
                ))}
              </>
            ) : (
