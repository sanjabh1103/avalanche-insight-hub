...
import { Menu, Zap, Layers } from 'lucide-react';
...
interface ForecastTopControlsProps {
  ...
  setMultiHazardEnabled?: (enabled: boolean) => void;
}
            ...
            <div className={`order-2 flex min-w-0 flex-wrap items-center gap-2 md:justify-end ${useWideTopControlLayout ? 'xl:order-3 xl:max-w-[16.5rem] xl:justify-start xl:justify-self-end' : ''}`}>
              ...
              {setMultiHazardEnabled && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="flex min-w-0 items-center gap-2 glass-panel rounded-2xl px-3 py-2.5 shadow-sm">
                      <Layers className={`h-3.5 w-3.5 ${multiHazardEnabled ? 'text-cyan-400' : 'text-muted-foreground'}`} />
                      <Label className="text-[10px] font-mono uppercase tracking-[0.22em] text-muted-foreground cursor-pointer" htmlFor="multi-hazard-toggle">Multi-Hazard</Label>
                      <Switch id="multi-hazard-toggle" checked={multiHazardEnabled ?? false} onCheckedChange={setMultiHazardEnabled} aria-label="Toggle Multi-Hazard Mode" />
                    </div>
                  </TooltipTrigger>
                  <TooltipContent>Show landslide, flood, and rockfall risk alongside avalanche</TooltipContent>
                </Tooltip>
              )}
            </div>
