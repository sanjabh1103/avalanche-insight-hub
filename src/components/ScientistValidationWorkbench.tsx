...
export default function ScientistValidationWorkbench({ gateStatuses = [] }: Props) {
  ...
  const PartnerPairedComparison = useMemo(() => {
    ...
    const modelOverconfident = reviewedCases.filter((c) => {
      const cellRisk = Number(c.cell_snapshot?.riskScore ?? 0);
        ...
        <div className="rounded-lg border border-sky-500/20 bg-sky-500/5 p-2">
          ...
          <div className="grid grid-cols-3 gap-1.5 text-[10px]">
            <div className="rounded-md border border-border/40 bg-black/10 px-2 py-1">
              ...
              <div className="font-mono text-sm text-foreground">{PartnerPairedComparison.totalReviewed}</div>
            </div>
            <div className="rounded-md border border-border/40 bg-black/10 px-2 py-1">
              <div className="text-[9px] uppercase tracking-[0.15em] text-muted-foreground">Agreement</div>
              <div className="font-mono text-sm text-foreground">{PartnerPairedComparison.agreementRate}%</div>
            </div>
            <div className="rounded-md border border-border/40 bg-black/10 px-2 py-1">
              <div className="text-[9px] uppercase tracking-[0.15em] text-muted-foreground">Disagreements</div>
              <div className="font-mono text-sm text-amber-300">{PartnerPairedComparison.disagreements}</div>
            </div>
            <div className="rounded-md border border-border/40 bg-black/10 px-2 py-1">
              <div className="text-[9px] uppercase tracking-[0.15em] text-muted-foreground">Model Over</div>
              <div className="font-mono text-sm text-red-300">{PartnerPairedComparison.modelOverconfident}</div>
            </div>
            <div className="rounded-md border border-border/40 bg-black/10 px-2 py-1">
              <div className="text-[9px] uppercase tracking-[0.15em] text-muted-foreground">Model Under</div>
              <div className="font-mono text-sm text-sky-300">{PartnerPairedComparison.modelUnderconfident}</div>
            </div>
            <div className="rounded-md border border-border/40 bg-black/10 px-2 py-1">
              <div className="text-[9px] uppercase tracking-[0.15em] text-muted-foreground">2-Reviewer Done</div>
              <div className="font-mono text-sm text-emerald-300">{PartnerPairedComparison.twoReviewerComplete}</div>
            </div>
          </div>
          {PartnerPairedComparison.twoReviewerPending > 0 && (
            <div className="mt-1 text-[10px] text-amber-300">
              {PartnerPairedComparison.twoReviewerPending} priority 5 case(s) awaiting second reviewer
            </div>
          )}
        </div>
