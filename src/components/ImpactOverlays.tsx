...
export default function ImpactOverlays({ bbox, showRoads, showInfrastructure }: Props) {
  ...
  useEffect(() => {
    ...
    if (showRoads) {
      queries.push(
        fetch(OVERPASS_URL, { method: 'POST', body: `data=${encodeURIComponent(buildQuery(bbox, 'roads'))}`, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
          .then(r => r.json())
          .then(data => {
            if (cancelled) return;
            (data.elements || []).forEach((el: Record<string, unknown>) => {
              if (el.type === 'way' && Array.isArray(el.geometry)) {
                const coords = (el.geometry as { lat: number; lon: number }[]).map(p => [p.lat, p.lon] as [number, number]);
    ...
    if (showInfrastructure) {
      queries.push(
        fetch(OVERPASS_URL, { method: 'POST', body: `data=${encodeURIComponent(buildQuery(bbox, 'infra'))}`, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
          .then(r => r.json())
          .then(data => {
            if (cancelled) return;
            (data.elements || []).forEach((el: Record<string, unknown>) => {
              if (el.type === 'node' && typeof el.lat === 'number' && typeof el.lon === 'number') {
                ...
                marker.bindPopup(`<b>${tags.name || (isLift ? 'Ski Lift' : 'Village')}</b><br/>Type: ${tags.aerialway || tags.place || 'unknown'}`);
                marker.addTo(map);
