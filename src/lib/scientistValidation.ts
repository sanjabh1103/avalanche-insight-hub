...
export interface DailyVerificationAnalytics {
  ...
  danger_disagreement_count: number;
  danger_disagreement_rate: number | null;
  outcome_linked_pair_count: number;
  event_observed_assessable_count: number;
  false_alarm_candidate_count: number;
  miss_candidate_count: number;
  high_danger_event_hit_count: number;
  high_danger_recall_proxy: number | null;
...
function isAssessableDangerLevel(value: DailyVerificationDangerLevel): boolean {
  return value !== 'not_assessed';
}

function isHighDanger(value: DailyVerificationDangerLevel): boolean {
...
export function buildDailyVerificationAnalytics(records: DailyVerificationRecord[]): DailyVerificationAnalytics {
  ...
  const disagreementQueue: DailyVerificationAnalytics['disagreement_queue'] = [];
  let assessableDangerPairCount = 0;
  let exactDangerMatches = 0;
  let outcomeLinkedPairCount = 0;
  let eventObservedAssessableCount = 0;
  let falseAlarmCandidateCount = 0;
  let missCandidateCount = 0;
  let highDangerEventHitCount = 0;
  ...
  records.forEach((record) => {
    ...
    if (isAssessableDangerLevel(scientistDanger) && isAssessableDangerLevel(modelDanger)) {
      assessableDangerPairCount += 1;
      if (scientistDanger === modelDanger) {
        exactDangerMatches += 1;
      } else {
        disagreementQueue.push({
          id: record.id,
          verification_date: record.verification_date,
          region: record.region_name ?? record.region_key ?? 'region n/a',
          scientist_danger_level: scientistDanger,
          model_danger_level: modelDanger,
          observed_outcome: observedOutcome,
        });
      }
    }

    if (isAssessableDangerLevel(modelDanger) && observedOutcome !== 'unknown') {
      outcomeLinkedPairCount += 1;
      if (observedOutcome === 'event_observed') {
        eventObservedAssessableCount += 1;
        if (isHighDanger(modelDanger)) {
          highDangerEventHitCount += 1;
        } else {
          missCandidateCount += 1;
        }
      }
      if (observedOutcome === 'no_event_observed' && isHighDanger(modelDanger)) {
        falseAlarmCandidateCount += 1;
      }
    }
  });

  const dangerDisagreementCount = assessableDangerPairCount - exactDangerMatches;
  return {
    ...
    danger_disagreement_count: dangerDisagreementCount,
    danger_disagreement_rate: assessableDangerPairCount ? dangerDisagreementCount / assessableDangerPairCount : null,
    outcome_linked_pair_count: outcomeLinkedPairCount,
    event_observed_assessable_count: eventObservedAssessableCount,
    false_alarm_candidate_count: falseAlarmCandidateCount,
    miss_candidate_count: missCandidateCount,
    high_danger_event_hit_count: highDangerEventHitCount,
    high_danger_recall_proxy: eventObservedAssessableCount ? highDangerEventHitCount / eventObservedAssessableCount : null,
    ...
    disagreement_queue: disagreementQueue,
    claim_boundary: 'Paired verification is comparison evidence only. It is not a local holdout metric report, does not promote public scoring, and does not replace official scientist sign-off.',
  };
}
