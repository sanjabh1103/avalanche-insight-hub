...
// ---------------------------------------------------------------------------
// F18: Physics narrative lexicon and helpers
// ---------------------------------------------------------------------------

export const PHYSICS_GRAIN_LABELS: Record<string, string> = {
...
export function physicsStabilityLabel(index: number | null | undefined): { label: string; tone: string } {
  if (typeof index !== 'number' || !Number.isFinite(index)) return { label: 'Unknown', tone: 'text-muted-foreground' };
  if (index < 1.0) return { label: 'Unstable', tone: 'text-red-400' };
  if (index < 1.5) return { label: 'Marginal', tone: 'text-amber-300' };
  return { label: 'Stable', tone: 'text-emerald-300' };
}

export function physicsShearLabel(kpa: number | null | undefined): { label: string; tone: string } {
  if (typeof kpa !== 'number' || !Number.isFinite(kpa)) return { label: 'Unknown', tone: 'text-muted-foreground' };
  if (kpa < 3) return { label: 'Weak', tone: 'text-red-400' };
  if (kpa < 5) return { label: 'Moderate', tone: 'text-amber-300' };
  return { label: 'Strong', tone: 'text-emerald-300' };
}

export function buildPhysicsExplanation(cell: GridCell): string {
  const pn = cell.physicsNarrative;
  if (!pn) {
    return 'Physics narrative unavailable for this forecast batch. Run a new inference job to populate.';
  }
  return pn.summary;
}
