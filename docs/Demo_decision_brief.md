...
- `Live demo` = visible on the current public route or live admin gate
...
|---|---|---|---:|---|---|---|---|
| 1 | Slow or brittle forecast delivery under heavy compute | `tackled` | 1 | The public app serves a precomputed 72 h forecast artifact with ready/partial/stale states and lazy hour loading. | It is fast because it is batch-first, not because heavy science is solved in real time. | `PRECOMPUTED BATCH • READY (72h)` badge, timeline, map load | [Forecast workspace](https://avalanche-insight-hub.netlify.app/) |
...
| 7 | Forecast reasoning must be inspectable | `tackled` | 2 | SHAP-driven cell inspection and operator provenance surfaces reduce black-box risk. | Explainability does not prove correctness, and some paths remain surrogate-based. | Cell inspection, expert controls, model provenance | [Forecast workspace](https://avalanche-insight-hub.netlify.app/) |
...
| 11 | Rapid update cadence from fresh evidence | `partial` | 3 | The product refreshes the latest published artifact and exposes freshness plus async jobs. | It is not a real-time autonomous retraining platform yet. | Freshness states, latest artifact language | [Forecast workspace](https://avalanche-insight-hub.netlify.app/) |
...
| PWA offline field-report replay | Repo/admin path | `Repo/admin verified` | Helps collect mountain evidence even under unreliable connectivity. | Research papers rarely address field-evidence UX and offline replay directly. | 3 | Useful operationally, but not a scientific novelty claim. |
...
|---|---|---|---|
| Forecast experience | 72 h batch workspace, structured bulletin, uncertainty cues, share/export/report | Better consequence workflows and cleaner operator analytics | Real-time self-updating autonomous forecast engine |
...
| Remote sensing | Honest SAR coverage signaling | SAR artifact promotion and controlled remote-sensing pilot | Universal operational SAR truth engine across all regions |
