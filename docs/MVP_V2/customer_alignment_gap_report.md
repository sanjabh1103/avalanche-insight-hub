...
- **Customer-Readiness Grade:** **B-** (Downgraded from B due to physical dry-snow limitations and SMOTE synthetic boundary distortion).
- **Overall Alignment Score:** **3.4 / 5.0** (Calibrated as a decision-support MVP moving toward pilot readiness).
- **Go/No-Go Recommendation:** **GO (with strict, mandatory Claim Boundaries)**. The platform is ready for a structured scientist-partner validation pilot, but **NO-GO** for operational warning or safety-critical deployment. All claims regarding SAR remote sensing, MTS-LSTM sequence modeling, and snowpack stratigraphic modeling must remain strictly bounded as gated/candidate paths.
...
### Top Gaps & Risks (Adversarial Critique Focus)
1. **Scientific Overclaim on Snowpack (1.5/5):** The "HIM-STRAT Class-II snowpack simulation" ([snowpack_proxy.py](file:///Users/sanjayb/avalanche-insight-hub/backend/common/snowpack_proxy.py)) is a simple weather-based cumulative heuristic, not a thermodynamic physical model. Presenting this as a physical stratigraphy simulation to scientific partners constitutes a major credibility risk.
2. **SAR Physical Blind Spots (2.0/5):** The Sentinel-1 SAR change detection pipeline (`sar_unet_training.py` and `sar_unet_worker.py`) does not gate predictions based on snow wetness or air temperature. C-band radar is transparent to dry snow; dry slab avalanches will result in false negatives. The assumption that adding more training scenes will solve the precision floor is over-optimistic—it is a physical limitation.
3. **Synthetic Weather Inconsistency (3.0/5):** In [surrogate_rf.py](file:///Users/sanjayb/avalanche-insight-hub/backend/models/surrogate_rf.py), applying `KMeansSMOTE` to weather and terrain features interpolates points that violate physical laws (e.g. high temperatures at high elevations during mid-winter), creating distorted decision boundaries.
4. **Gateway JWT Bypasses (3.5/5):** The [config.toml](file:///Users/sanjayb/avalanche-insight-hub/supabase/config.toml) disables gateway-level JWT verification (`verify_jwt = false`) for all 9 Edge Functions. While functions are protected at the Deno handler level via [auth.ts:authorizeJobRequest](file:///Users/sanjayb/avalanche-insight-hub/supabase/functions/_shared/auth.ts#L44), this increases security surface vulnerability if new routes are added without manual handler-level checks.
5. **Insecure Default Admin Credentials (3.5/5):** The login page pre-populates the admin email `admin@insight-hub.local` with default credentials (`test123` in `.env`), creating a critical risk if deployed to production without rotation.
...
| Category | Score | Code/Doc Evidence | Gap Analysis & Adversarial Critique | Risk to Customer Send | Recommended Action |
| :--- | :---: | :--- | :--- | :--- | :--- |
...
| **2. Same-Day Freshness Proof** | **4.0** | [run-forecast/index.ts:L92-111](file:///Users/sanjayb/avalanche-insight-hub/supabase/functions/run-forecast/index.ts#L92-111) | Edge function successfully checks `forecastDate === today` and calculates `freshnessHours <= 24` to mark `sameDayPublished`. | Automating same-day batch generation on every demo cycle requires constant execution. Faking freshness via "trigger scripts" to make the demo look good hides pipeline failures. | Demonstration displays stale warnings if the daily GHA cron has not completed. | Implement clear data latency displays instead of faking same-day runs. |
...
| **5. Claim Honesty & Deck QA** | **3.0** | [Speaker_Notes_Deckwise.md](file:///Users/sanjayb/avalanche-insight-hub/docs/MVP/presentation/Speaker_Notes_Deckwise.md) | Slide outline relies heavily on "current state" and "future strategy" separation. | Deck still contains overclaims regarding "HIM-STRAT physical snowpack simulation" and active SAR warnings. | Partner scientists will reject the platform if they discover these overclaims. | Perform a strict revision of the slides to re-label the snowpack proxy and gate SAR. |
...
| **7. SAR & Remote-Sensing Path** | **2.0** | [sar_unet_training.py](file:///Users/sanjayb/avalanche-insight-hub/backend/sar_unet_training.py), [sar_acceptance_policy.py](file:///Users/sanjayb/avalanche-insight-hub/backend/common/sar_acceptance_policy.py) | Segmentation model scripts are present but marked `blocked_shadow_only`. | Dry snow transparency to C-band SAR is not addressed, leading to false negatives in dry climates. | Falsely claiming operational satellite avalanche warnings will lead to safety risks. | Frame Sentinel-1 SAR as a retrospective calibration tool, not a real-time warning system. |
...
| **9. Snowpack Simulation & Science** | **1.5** | [snowpack_proxy.py](file:///Users/sanjayb/avalanche-insight-hub/backend/common/snowpack_proxy.py) | Weather values are fetched cumulatively from Nov 1 to compute shear and settlement. | The code implements a cumulative weather heuristic, not a thermodynamic stratigraphic model. | Presenting this as "HIM-STRAT physical simulation" is scientifically misleading. | Re-label "HIM-STRAT simulation" to "Weather-Driven Heuristic Proxy" in all decks and UI cards. |
| **10. Class Imbalance & Metrics** | **3.5** | [surrogate_rf.py](file:///Users/sanjayb/avalanche-insight-hub/backend/models/surrogate_rf.py) | Implements Peirce Skill Score (PSS) optimization, time-series split, and calibrated classifier CV. | KMeansSMOTE on weather/terrain features interpolates unphysical states (e.g. warm temperatures at high elevations). | Distorted decision boundaries in unphysical parts of the feature space. | Evaluate the physical consistency of the Random Forest's decision boundaries. |
...
| **12. Security & Pilot Readiness** | **3.5** | [config.toml](file:///Users/sanjayb/avalanche-insight-hub/supabase/config.toml), [auth.ts:L44-100](file:///Users/sanjayb/avalanche-insight-hub/supabase/functions/_shared/auth.ts#L44-100) | Gating is enforced inside Deno handlers for all mutating/cost-incurring paths. | Disabling `verify_jwt` globally creates a single-point-of-failure risk if new endpoints omit manual checks. | A developer adding a write route and forgetting the auth helper exposes the DB. | Separate Edge Functions into public user endpoints (verify_jwt=true) and system webhooks. |
...
* **Impact:** Disabling gateway verification removes Supabase's native Kong protector. A single handler that fails to call `authorizeJobRequest` or has a logical error will expose database write operations to anonymous public callers.
...
graph TD
    ...
    G -->|Hourly & Daily Sequences| H[MTS-LSTM PyTorch Model]
...
## 8. Resolved Pilot Decisions & Parameters

1. **Snowpack Upgrade Path (Resolved):** For the immediate pilot, we continue using the cumulative weather heuristic but have explicitly renamed it to the **Weather-Driven Heuristic Proxy** across the UI and documentation to prevent scientific overclaims. For the next phase, the target architecture explicitly requires driving 1D multi-layer physical thermodynamic models (such as Swiss WSL/SLF `SNOWPACK` or `SAFRAN-CROCUS` for layer/crystal metamorphism) using downscaled high-resolution Numerical Weather Prediction (NWP) outputs.
2. **SAR Precision Thresholds (Resolved):** We will maintain the strict precision floor gates and keep the SAR path in `blocked_shadow_only` status to prioritize safety. Lowering the precision floor is rejected as a dangerous anti-pattern due to Sentinel-1 dry-snow transparency and steep Himalayan terrain distortions. We will maintain this shadow gate until a multi-orbital/multi-modal (ascending/descending pass) fusion is implemented to resolve radar shadows without compromising the precision floor.
3. **Admin Cleanout (Resolved):** Since the repository is currently private, the default login configuration (`admin@insight-hub.local` / `test123`) is permitted to remain in `.env` and pre-populate the login page for developer convenience during the initial pilot phase, but must be strictly rotated and cleared before making the repository public.
