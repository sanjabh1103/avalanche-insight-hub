...
|---|---|---|---|---|
| Is the last three weeks of work worth taking to the scientist now? | `Conditional go` | Yes, if the meeting is positioned around governed decision support, proof-tier honesty, and co-development opportunity rather than finished autonomy. | `docs/Demo_decision_brief.md` decision synthesis; `docs/Cust_comm.md` expectations 3, 10, 13, 14, 15; live route `/` | Scientist trust will drop quickly if the discussion sounds like autonomy-first marketing. |
| Can we pitch this as advanced autonomous avalanche science already delivered? | `No-go` | No. The current repo does not support `Active` claims for production MTS-LSTM, authoritative SAR, authority-grade warning status, or fully autonomous truth generation. | `docs/Demo_research_appendix.md` repo-vs-claim audit; `docs/OPERATOR_ROLLOUT_QA.md` release communication gate; `/admin` gate only | A senior scientist will correctly read this as overclaiming and may disengage from the rest of the proposal. |
| Is there a scientifically respectable discussion worth having now? | `Yes` | The strongest current value is a disciplined operational shell: batch-first forecast delivery, masking honesty, uncertainty communication, explainability, and governance posture. | `docs/Demo_decision_brief.md` present MVP gap analysis and top-5 methodology comparison; route `/`; `src/pages/Index.tsx` behavior contract | If this is hidden behind weak AI language, the actual operational strengths may be missed. |
| Can this discussion plausibly lead to immediate co-development interest? | `Yes, with the right framing` | Experienced scientists are more likely to engage around benchmark ownership, validation authority, and critical-layer review loops than around UI novelty alone. | `docs/Demo_decision_brief.md` future path and publication snapshot; `docs/Demo_research_appendix.md` publication pros/cons | Without a concrete scientist role, the meeting can degrade into a one-way demo instead of a partnership discussion. |
...
| 8 | Open, inspectable co-development stack | `Conditionally available` | `Repo/admin verified` | The unique value is not the tools themselves, but that the whole product is auditable and can support shared scientific ownership. | Experienced scientists are more likely to engage when they can inspect assumptions, artifacts, and limits directly. | Open repo structure; `docs/Demo_decision_brief.md` publication snapshot; `docs/Top20_features.md` open stack row | “This is a stack that your scientist team can inspect, challenge, and help evolve rather than a sealed vendor black box.” | 4 |
...
|---|---|---|---|---|---|
| 1 | Establish the real problem and why sparse-data avalanche forecasting remains hard | One short challenge summary from `docs/Demo_decision_brief.md` plus the top challenge rows from `docs/Top_challanges.md` | “We are not assuming avalanche forecasting is a solved ML problem; we are starting from the real sparse-data and validation bottlenecks.” | Do not open with product UI or “AI breakthrough” language. | The scientist agrees the problem framing is serious and grounded. |
| 2 | Show what the MVP truthfully proves now | Route `/`, batch badge, bulletin framing, masking, uncertainty cues, share/export/report, expert overlays | “Here is what the current product genuinely proves as a decision-support shell.” | Do not imply production autonomy, official warning-service status, or operational SAR. | The scientist can see current proof without overclaim. |
| 3 | Show what is promising but still gated | Candidate-model language, admin/model governance posture, governed ingest direction, research appendix caveats | “These are credible candidate directions, but they are not active claims yet.” | Do not say `production MTS-LSTM`, `authoritative SAR`, `fully autonomous avalanche AI`, or `zero-history learning`. | The boundary between current and future is understood. |
