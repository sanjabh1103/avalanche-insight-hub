-- Wire the existing stale-run marker into the daily retention control plane.
-- Use updated_at rather than created_at so an actively progressing run is not
-- marked failed merely because it was queued long ago.
...
BEGIN
  ...
  SET status = 'failed',
      error = 'Stale run — no status update within 30 days'
...
COMMENT ON FUNCTION public.cleanup_forecast_retention(interval, integer, integer) IS
  'Marks stale SNOWPACK runs and deletes bounded stale SHAP/grid/run data while preserving active, ready, validated, published, and outcome-referenced evidence.';
