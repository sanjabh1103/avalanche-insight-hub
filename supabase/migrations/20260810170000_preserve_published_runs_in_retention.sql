-- Preserve publication history when bounded retention removes stale runs.
--
-- 20260810163000 protects active/published grids, but its separate forecast_runs
-- delete must also preserve published/validated/ready run records. Removing
-- those rows can break historical links and publication audit history even when
-- the compatibility grid remains available.
...
COMMENT ON FUNCTION public.cleanup_forecast_retention(interval, integer, integer) IS
  'Deletes bounded stale SHAP/grid/run data while preserving active, ready, validated, published, and outcome-referenced evidence.';
