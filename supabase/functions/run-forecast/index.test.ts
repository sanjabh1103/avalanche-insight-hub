...
Deno.test("handleRunForecast marks a missing published manifest unavailable", async () => {
  const response = await handleRunForecast(
    new Request("http://localhost", {
      method: "POST",
      body: JSON.stringify({ regionKey: "colorado_rockies" }),
    }),
    {
      fetchFreshRun: async () => ({
        data: {
          id: "run-missing-manifest",
          region_name: "Colorado Rockies",
          region_key: "colorado_rockies",
          forecast_date: "2026-05-01",
          horizon_hours: 72,
          manifest_storage_ref: "forecast-products/avalanche/colorado/run-missing/manifest.json",
          compatibility_forecast_grid_id: "fg-missing-manifest",
          forecast_bulletins: null,
      ...
      fetchLatestGrid: async () => ({ data: null, error: null }),
      checkManifestExists: async () => false,
      getToday: () => "2026-05-01",
      getNow: () => new Date("2026-05-01T04:00:00.000Z"),
    },
  );
  ...
  assertEquals(payload.forecastRunId, "run-missing-manifest");
  assertEquals(payload.manifestPath, "forecast-products/avalanche/colorado/run-missing/manifest.json");
});
