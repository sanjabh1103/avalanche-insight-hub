...
export async function authorizeJobRequest(
  ...
  // Check if REQUIRE_JOB_AUTH is disabled
  const requireJobAuth = Deno.env.get("REQUIRE_JOB_AUTH");
  if (requireJobAuth === "false" || requireJobAuth === "0" || requireJobAuth === "off" || requireJobAuth === "no") {
    return { authorized: true, audit: { source: "disabled" } };
  }

  const callerAuthorization = req.headers.get("authorization");
  const jobTokenHeader = req.headers.get("x-job-token");
  const token = extractBearerToken(callerAuthorization) || jobTokenHeader;
