...
import { isGeminiSpendCapExceeded, incrementGeminiUsage, reserveGeminiUsage } from "../_shared/auth.ts";
...
async function generateGeminiSummary(supabase: any, features: TopFeature[], riskScore: number, problemType: string): Promise<string | null> {
  ...
  try {
    // Atomic reservation: increment usage only if under cap (fail-closed)
    const reserved = await reserveGeminiUsage(supabase);
    if (!reserved) {
      console.warn("[shap-explainer] Gemini spend cap exceeded. Falling back to deterministic summary.");
      return null;
    }
  } catch (capErr) {
    console.error("[shap-explainer] Spend cap reservation failed:", (capErr as Error).message);
    return null;
  }
