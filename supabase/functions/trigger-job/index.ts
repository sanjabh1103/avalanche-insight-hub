...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (type === 'daily_enrichment') {
      ...
      if (modalResult?.feature_weights && typeof modalResult.feature_weights === 'object' && !Array.isArray(modalResult.feature_weights)) {
        newOptimizationVersion = `opt-gpu-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}`;
        optimizationSummary = {
          ...
          selected_features: Array.isArray(modalResult.selected_features)
            ...
            : Object.keys(modalResult.feature_weights as Record<string, unknown>),
        ...
        newOptimizationVersion = typeof currentSummary?.optimization_version === 'string'
          ? currentSummary.optimization_version
          : currentOptimizationVersion;
      } else {
        newOptimizationVersion = `opt-edge-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}`;
        optimizationSummary = {
          optimization_version: newOptimizationVersion,
          feature_weights: {
            snowfall_24h: 0.24,
          ...
          selected_features: ['snowfall_24h','wind_loading','slope','elevation','temp_gradient','snowpack','ram_hardness','shear_strength','settlement_rate','aspect_loading'],
          class_balance_report: { strategy: 'edge-lite-resampling', false_negative_penalty: 4 },
          abc_enabled: false,
          runtime_mode: capabilities.mode,
